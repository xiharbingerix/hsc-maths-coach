...
} from "../../../lib/lessons/trigonometricFunctionsGraphs";
...
const unitCoverage = [
  ...
  "The tangent graph and asymptotes",
...
export default function TrigonometricFunctionsGraphsModulePage() {
  const activeCount = trigonometricFunctionsGraphsLessons.length;
  const plannedCount = trigonometricFunctionsGraphsOutline.length;
      ...
      <section className="mx-auto max-w-5xl space-y-8">
        <header className="rounded-2xl bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-slate-600">
            A structured pathway through radians, exact trigonometric values,
            unit-circle reasoning, and the key features of sine, cosine, and
            tangent graphs.
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-6">
            <section className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-3 text-slate-700">
                <li>Start with radians and exact values before graph work.</li>
                <li>Use the mastery quiz to check whether key values are secure.</li>
                <li>If you miss the pass mark, work through the full staged lesson.</li>
                <li>Return to unit-circle signs when graph or equation questions feel uncertain.</li>
              </ul>
            </section>
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-6">
              {trigonometricFunctionsGraphsOutline.map((lesson, index) => {
                const isActive = lesson.status === "active";
                const isLast =
                  index === trigonometricFunctionsGraphsOutline.length - 1;
                const content = (
                  <div
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`${isLast ? "" : "pb-5"}`}>
                      {isActive ? (
                        <Link
                          href={`/course/trigonometric-functions-graphs/${lesson.slug}`}
                          className="block"
                        >
                          {content}
                        </Link>
