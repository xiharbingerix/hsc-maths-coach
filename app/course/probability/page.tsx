...
const unitCoverage = [
  ...
  "Two-way tables",
  "Mixed HSC-style probability practice",
];

export default function ProbabilityModulePage() {
  const activeCount = probabilityLessons.length;
  const plannedCount = probabilityOutline.length;
      ...
      <section className="mx-auto max-w-5xl space-y-8">
        <header className="rounded-2xl bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-slate-600">
            A pathway through the NSW HSC Advanced probability strand: axioms,
            Venn diagrams, conditional probability, tree diagrams, independence,
            the multiplication rule, and two-way tables.
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-6">
            <section className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-3 text-slate-700">
                <li>Start with basics and Venn diagrams before moving to conditional probability.</li>
                <li>Draw a tree diagram or Venn diagram before applying any formula.</li>
                <li>Always identify whether events are independent or dependent first.</li>
                <li>Use complement (1 − P(none)) for "at least one" questions.</li>
              </ul>
            </section>
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-6">
              {probabilityOutline.map((lesson, index) => {
                const isActive = lesson.status === "active";
                const isLast = index === probabilityOutline.length - 1;
                const content = (
                  <div
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`${isLast ? "" : "pb-5"}`}>
                      {isActive ? (
                        <Link
                          href={`/course/probability/${lesson.slug}`}
                          className="block"
                        >
                          {content}
                        </Link>
