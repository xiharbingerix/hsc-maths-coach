...
} from "../../../lib/lessons/furtherTrigonometry";
...
export default function FurtherTrigonometryModulePage() {
  const activeCount = furtherTrigonometryLessons.length;
  const plannedCount = furtherTrigonometryOutline.length;
      ...
      <section className="mx-auto max-w-5xl space-y-8">
        <header className="rounded-2xl bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-slate-600">
            A HSC-focused unit for compound angle formulas, double angle
            formulas, exact values, identity simplification and further
            trigonometric equation work.
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-6">
            <section className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-3 text-slate-700">
                <li>Review exact values and quadrant signs before starting.</li>
                <li>Focus on choosing the identity before simplifying.</li>
                <li>Use multiple-choice options to compare signs and forms.</li>
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-6">
              {furtherTrigonometryOutline.map((lesson, index) => {
                const isActive = lesson.status === "active";
                const isLast = index === furtherTrigonometryOutline.length - 1;
                const content = (
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`${isLast ? "" : "pb-5"}`}>
                      {isActive ? (
                        <Link
                          href={`/course/further-trigonometry/${lesson.slug}`}
                          className="block"
                        >
                          {content}
                        </Link>
