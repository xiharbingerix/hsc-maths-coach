...
async function sendPurchasePrompt(formData: FormData) {
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? "https://novamaths.com.au";

  await getResend().emails.send({
    // Update this to a domain verified in your Resend account.
    from: "Nova Maths <hello@novamaths.com.au>",
    to: toEmail,
    subject: "Your Nova Maths account is ready — start learning today",
    html: buildPurchasePromptEmail({ studentName, siteUrl }),
  });
}

function buildPurchasePromptEmail({
  studentName,
  siteUrl,
}: {
  studentName: string;
