...
const algebraicNotation: LessonContent = {
  ...
  teaching: {
    paragraphs: [
      "Algebra is just arithmetic with a blank left in it. Instead of writing a number we do not yet know, we write a letter — and a letter that holds the place of a number is called a pronumeral. In $5x$, the letter $x$ is a pronumeral: it could be 2, or 100, or any number at all. The point of using a letter is that one statement can then describe every case at once.",
      "Here is the concrete picture. Suppose a movie ticket costs \\$$x$. The cost of one ticket is $x$, the cost of two is $x + x$, and the cost of five is $x + x + x + x + x$. Rather than write five $x$'s, we record 'five lots of $x$' as $5x$. So $5x$ is not a new mystery symbol — it is plain repeated addition, written short.",
      "That gives us the first piece of notation. When a number multiplies a pronumeral we drop the multiplication sign and write them side by side: $3 \\times x$ becomes $3x$. When two different letters multiply, the same thing happens: $a \\times b$ becomes $ab$. When a letter multiplies itself, we use a power: $a \\times a$ is $a^2$ ('a squared'), and $a \\times a \\times a$ is $a^3$ ('a cubed'). The small raised number just counts how many copies are multiplied together.",
      "Why drop the multiplication sign at all? Because in algebra the cross $\\times$ looks almost exactly like the letter $x$, so $3 \\times x$ would read as $3xx$ — a real source of confusion. Writing $3x$ removes the clash entirely. And why is $a^2$ different from $2a$? Because $a^2$ means $a \\times a$ (a multiplied by itself), while $2a$ means $a + a$ (two copies added). If $a = 5$ then $a^2 = 25$ but $2a = 10$ — completely different numbers, so the notation must keep them apart.",
      "Now the building blocks. A term is a single piece built only from multiplication and powers — a number, a pronumeral, or a product like $4x$ or $3ab$. Terms are joined into an expression by $+$ and $-$ signs: $2x + 5$ is an expression with two terms. To count the terms, count the pieces the $+$ and $-$ signs cut the expression into. An equation goes one step further: it places an $=$ sign between two expressions to claim they are equal, like $2x + 5 = 11$.",
      "The difference between an expression and an equation matters. An expression such as $2x + 5$ has no equals sign — it is a recipe waiting for a value of $x$, and on its own there is nothing to 'solve'. An equation such as $2x + 5 = 11$ does make a claim, so it can be solved to find which $x$ makes it true. A common slip is to think $2x + 5$ already 'equals' something; it does not until you are told $x$.",
      "Translating words into algebra is where order trips people up. 'Five more than a number $n$' is $n + 5$ — addition does not care about order, so that one is safe. But 'a number $n$ less than five' is $5 - n$, not $n - 5$: you start at five and take $n$ away. The phrase 'less than' names the amount removed, and it comes after the thing you start from, so read the whole phrase before you write a sign.",
    ],
    latexBlocks: [
      "3 \\times x = 3x, \\quad a \\times b = ab, \\quad a \\times a = a^2",
      "a^2 = a \\times a \\quad (\\text{not } 2a = a + a)",
      "\\underbrace{4x}_{\\text{term}} + \\underbrace{3}_{\\text{term}} \\quad \\text{(expression: 2 terms, no } = \\text{)}",
      "4x + 3 = 11 \\quad \\text{(equation — has an } = \\text{ sign)}",
    ],
  },
  workedExamples: [
    {
      ...
      steps: [
        { explanation: "A number times a pronumeral: drop the multiplication sign so it cannot be misread as the letter x.", latex: "5 \\times n = 5n" },
        { explanation: "A pronumeral times itself is repeated multiplication, so record it as a power — here, squared.", latex: "p \\times p = p^2" },
        { explanation: "Drop all the signs, then by convention write the number first and the letters in alphabetical order.", latex: "4 \\times a \\times b = 4ab" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "'Three times a number n' means three lots of n, which is the product 3 times n.", latex: "3 \\times n = 3n" },
        { explanation: "'Four more than' that result means add 4 to it.", latex: "3n + 4" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Terms are the pieces separated by + and - signs, so split the expression at those signs.", latex: "5x^2 \\;\\big|\\; +\\,3x \\;\\big|\\; -\\,7" },
        { explanation: "There are three separate pieces, so the expression has three terms.", latex: "\\text{Number of terms} = 3" },
      ],
      finalAnswerLatex: "\\text{3 terms}",
    },
    {
      title: "Translate a multi-step word phrase (harder)",
      questionLatex: "\\text{A number is } m. \\text{ Write an expression for: the square of } m, \\text{ decreased by twice } m, \\text{ then state how many terms it has.}",
      steps: [
        { explanation: "'The square of m' means m multiplied by itself, written as a power.", latex: "m^2" },
        { explanation: "'Twice m' is two lots of m, and 'decreased by' means subtract it.", latex: "m^2 - 2m" },
        { explanation: "Distinguish the pieces: m squared and 2m are joined by one minus sign, so there are two terms.", latex: "\\underbrace{m^2}_{\\text{term 1}} - \\underbrace{2m}_{\\text{term 2}}" },
      ],
      finalAnswerLatex: "m^2 - 2m \\quad (\\text{2 terms})",
    },
  ],
  guidedPractice: [
    choice(
...
const collectingLikeTerms: LessonContent = {
  ...
  teaching: {
    paragraphs: [
      "Collecting like terms is how we tidy a long expression into a short one without changing its value. The idea is simple: pieces that measure the same thing can be added into a single piece, while pieces that measure different things must stay separate.",
      "Think of the pronumeral as a label, like a unit of money. If you have 3 ten-dollar notes and 5 ten-dollar notes, you have 8 ten-dollar notes — you just count the notes because they are all the same kind. In algebra, $3x + 5x = 8x$ for exactly the same reason: $x$ is the 'kind', and you have $3 + 5 = 8$ of them. But 3 ten-dollar notes and 5 five-dollar notes cannot be merged into '8 notes' of one kind — and likewise $3x + 5y$ cannot be merged, because $x$ and $y$ are different kinds.",
      "Now the names. The number in front of a term is its coefficient, and the letters-and-powers part is the pronumeral part. Like terms are terms with exactly the same pronumeral part — the same letter or letters raised to the same powers. So $3x$ and $7x$ are like terms (both are 'lots of $x$'), but $3x$ and $3x^2$ are not, because $x$ and $x^2$ are different kinds.",
      "Here is why the rule really holds, with no hand-waving. $3x$ literally means $x + x + x$ and $5x$ means $x + x + x + x + x$. Add them and you simply have eight $x$'s in a row: $x + x + x + x + x + x + x + x = 8x$. So 'add the coefficients, keep the pronumeral' is not an invented rule — it is just counting how many copies of $x$ you ended up with.",
      "The same counting argument shows why the power cannot change. $3x + 5x$ is eight copies of $x$, which is $8x$ — it is never $8x^2$, because we added the $x$'s, we did not multiply them. Multiplying eight $x$'s together would give $x^8$; adding eight of them gives $8x$. Collecting like terms only ever changes the count out front.",
      "When an expression mixes kinds, deal with one kind at a time. In $4a + 2b + 3a - b$, gather the $a$-terms ($4a$ and $3a$) and the $b$-terms ($2b$ and $-b$) into separate groups, then count each: $4a + 3a = 7a$ and $2b - b = b$, giving $7a + b$. A useful safeguard here is that a lone letter has an invisible coefficient of 1, so $b$ means $1b$ and $2b - b = 2b - 1b = b$.",
      "The error to watch for is forcing unlike terms together to make the answer shorter, for example writing $3x + 2y = 5xy$. The money picture explains why that is wrong: combining different kinds invents a quantity that was never there. If the terms are not the same kind, the simplest correct form is to leave them side by side — $3x + 2y$ is already fully simplified.",
    ],
    latexBlocks: [
      "3x + 5x = \\underbrace{x+x+x}_{3} + \\underbrace{x+x+x+x+x}_{5} = 8x",
      "3x + 5x = 8x \\quad (\\text{not } 8x^2) \\qquad 7a - 2a = 5a",
  ...
  workedExamples: [
    {
      ...
      steps: [
        { explanation: "All three terms have the same pronumeral part x, so they are like terms and can be combined.", latex: "5x,\\; 3x,\\; 2x \\text{ are all like terms}" },
        { explanation: "Counting copies of x, work through the coefficients in order and keep the x.", latex: "5 + 3 - 2 = 6" },
        { explanation: "The result is six copies of x.", latex: "5x + 3x - 2x = 6x" },
      ],
      finalAnswerLatex: "6x",
    },
    {
      ...
      steps: [
        { explanation: "Gather the a-terms, since only same-kind terms combine.", latex: "4a + 2a = 6a" },
        { explanation: "Gather the b-terms, remembering a lone -b means -1b.", latex: "3b - b = 3b - 1b = 2b" },
        { explanation: "The a-group and b-group are different kinds, so leave them side by side.", latex: "4a + 3b + 2a - b = 6a + 2b" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "x squared and x are different kinds (different powers), so sort the expression into two groups.", latex: "(3x^2 + 2x^2) + (5x - x)" },
        { explanation: "Count each group separately, keeping its own power.", latex: "3x^2 + 2x^2 = 5x^2, \\quad 5x - x = 4x" },
        { explanation: "Write the two unlike groups together; they cannot be merged further.", latex: "3x^2 + 5x + 2x^2 - x = 5x^2 + 4x" },
      ],
      finalAnswerLatex: "5x^2 + 4x",
    },
    {
      title: "Collect from a perimeter, three kinds mixed (harder)",
      questionLatex: "\\text{Simplify }\\; 6a + 4b - 2a + 7 - b - 3.",
      steps: [
        { explanation: "Identify the three kinds present: a-terms, b-terms, and plain numbers (constants).", latex: "(6a, -2a), \\;\\; (4b, -b), \\;\\; (7, -3)" },
        { explanation: "Count the a-terms.", latex: "6a - 2a = 4a" },
        { explanation: "Count the b-terms, using -b = -1b.", latex: "4b - b = 3b" },
        { explanation: "Count the constants, which are themselves a like-kind group.", latex: "7 - 3 = 4" },
        { explanation: "Write the three unlike groups side by side.", latex: "6a + 4b - 2a + 7 - b - 3 = 4a + 3b + 4" },
      ],
      finalAnswerLatex: "4a + 3b + 4",
    },
  ],
  guidedPractice: [
    choice(
...
const expandingBrackets: LessonContent = {
  ...
  teaching: {
    paragraphs: [
      "Expanding a bracket means rewriting a product like $3(x + 4)$ as a sum without the bracket. The rule, called the distributive law, is that the factor sitting outside the bracket multiplies every single term inside it — so $3(x + 4)$ becomes $3 \\times x$ plus $3 \\times 4$, which is $3x + 12$.",
      "You can see why with a picture of area. A rectangle that is 3 tall and $(x + 4)$ wide has its width split into a part of length $x$ and a part of length 4. Its total area can be found two ways: all at once as $3 \\times (x + 4)$, or as the two smaller rectangles $3 \\times x$ and $3 \\times 4$ added together. Both must give the same area, so $3(x + 4) = 3x + 12$. The bracket is just the whole rectangle; expanding is slicing it into pieces.",
      "In symbols the rule is written $a(b + c) = ab + ac$. The letter $a$ is the factor outside; $b$ and $c$ are the terms inside. The word that does all the work is every: the outside factor reaches each inside term, not just the first. Forgetting the second term — writing $3(x + 4) = 3x + 4$ — is the single most common error, and the area picture shows why it is wrong: you would be leaving out the whole $3 \\times 4$ rectangle.",
      "When the outside factor is negative, the same rule applies, but you must carry the minus sign onto every product. This is just the rule for multiplying signed numbers: a negative times a positive is negative, and a negative times a negative is positive. So in $-2(x - 5)$, the first product is $-2 \\times x = -2x$, and the second is $-2 \\times (-5) = +10$, giving $-2x + 10$. The second sign flips precisely because two negatives multiply to a positive.",
      "When an expression has two brackets, expand each one on its own first, then collect like terms — you already know how to do both halves. For $4(x + 1) + 2(x + 3)$, expanding gives $4x + 4 + 2x + 6$, and collecting the $x$-terms and the constants gives $6x + 10$. Splitting the work into 'expand, then collect' keeps long questions under control.",
      "One transfer warning for later: a constant multiplying a number means just that — multiply. A frequent slip is to add instead, writing $3(x + 5) = 3x + 8$ by doing $3 + 5$. But the bracket means three lots of $(x + 5)$, so the 5 is multiplied, giving $3 \\times 5 = 15$ and the correct $3x + 15$.",
      "At Year 7 every bracket here is a single bracket, and each inside term is one pronumeral or a constant. Products of two brackets such as $(x + 2)(x + 3)$ come later — do not use double brackets in this lesson.",
    ],
    latexBlocks: [
      "a(b + c) = ab + ac \\quad (\\text{the factor reaches every term})",
      "3(x + 4) = 3x + 12 \\quad (\\text{area: } 3\\cdot x \\text{ plus } 3\\cdot 4)",
      "-a(b - c) = -ab + ac \\quad (\\text{negative} \\times \\text{negative} = \\text{positive})",
  ...
  workedExamples: [
    {
      ...
      steps: [
        { explanation: "The factor 5 reaches the first term inside, so multiply them.", latex: "5 \\times 3x = 15x" },
        { explanation: "The same factor 5 must also reach the second term inside.", latex: "5 \\times 2 = 10" },
        { explanation: "Add the two products to get the expanded form.", latex: "5(3x+2) = 15x + 10" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Carry the minus onto the first product: negative times positive is negative.", latex: "-3 \\times 2x = -6x" },
        { explanation: "On the second product two negatives meet, so the result is positive.", latex: "-3 \\times (-4) = +12" },
        { explanation: "Add the two products, keeping their signs.", latex: "-3(2x-4) = -6x + 12" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Expand the first bracket on its own.", latex: "2(x+3) = 2x + 6" },
        { explanation: "Expand the second bracket on its own.", latex: "4(x-1) = 4x - 4" },
        { explanation: "Write both expansions in one line, ready to collect.", latex: "2x + 6 + 4x - 4" },
        { explanation: "Collect the like terms: x-terms together, constants together.", latex: "(2x+4x) + (6-4) = 6x + 2" },
      ],
      finalAnswerLatex: "6x + 2",
    },
    {
      title: "Expand with a subtracted bracket (harder)",
      questionLatex: "\\text{Expand and simplify }\\; 5(2x + 4) - 3(x - 2).",
      steps: [
        { explanation: "Expand the first bracket, where every product is positive.", latex: "5(2x+4) = 10x + 20" },
        { explanation: "The factor on the second bracket is -3, so carry the minus onto each product.", latex: "-3 \\times x = -3x, \\quad -3 \\times (-2) = +6" },
        { explanation: "Write the second expansion; note the constant became +6, not -6.", latex: "-3(x - 2) = -3x + 6" },
        { explanation: "Put both expansions together.", latex: "10x + 20 - 3x + 6" },
        { explanation: "Collect the x-terms and the constants.", latex: "(10x - 3x) + (20 + 6) = 7x + 26" },
      ],
      finalAnswerLatex: "7x + 26",
    },
  ],
  guidedPractice: [
    choice(
...
const factorisingCommonFactors: LessonContent = {
  ...
  teaching: {
    paragraphs: [
      "Factorising is expanding run backwards. Expanding takes $3(x + 4)$ and opens it into $3x + 12$; factorising takes $3x + 12$ and packs it back into $3(x + 4)$. Because it is the exact reverse, you already know the target shape — a factor outside a bracket — and the job is to discover what that factor is.",
      "The thing you pull outside is whatever the terms share. Look at $3x + 12$: both $3x$ and $12$ can be divided by 3, so 3 is a shared factor and can be lifted out the front. We pull out the largest such number, the highest common factor (HCF), because anything smaller would leave the bracket still able to be factorised further.",
      "Why does lifting it out work? It is the distributive law read in reverse. Forwards, $3$ multiplies into both $x$ and $4$ to give $3x + 12$. So if every term already has a factor of 3, that 3 must have come from a multiplication — and we can simply undo it. Concretely, $3x = 3 \\times x$ and $12 = 3 \\times 4$, so $3x + 12 = 3 \\times x + 3 \\times 4 = 3(x + 4)$. Nothing is added or removed; the 3 is just written once instead of twice.",
      "So the method has three moves. First find the HCF of the coefficients — the largest number dividing all of them. Then check the letters: if a pronumeral appears in every term, it is part of the HCF too. Finally, divide each term by the HCF to find what remains inside the bracket. For $6x + 10$, the coefficients 6 and 10 share 2 and there is no common letter, so the HCF is 2; dividing gives $3x + 5$ inside, hence $6x + 10 = 2(3x + 5)$.",
      "When a pronumeral is shared it joins the HCF. In $6x + 9x^2$, the numbers 6 and 9 share 3, and an $x$ sits in both terms, so the HCF is $3x$. Dividing, $6x \\div 3x = 2$ and $9x^2 \\div 3x = 3x$, giving $3x(2 + 3x)$. The watch-point here is a term that divides away completely: in $5x + 5$, dividing the second 5 by 5 leaves 1, not nothing, so the answer is $5(x + 1)$ — never $5(x)$.",
      "Always verify by expanding back. If $2(3x + 5)$ multiplies out to $6x + 10$, the factorisation is right; if it does not, you have made a slip — usually a wrong HCF or a forgotten 1. The check costs only a few seconds and catches the mistake before it spreads.",
      "Finally, make sure you have taken out the highest factor, not just a factor. Writing $12x + 18 = 3(4x + 6)$ is true but not finished, because $4x$ and $6$ still share a 2; the full factorisation is $6(2x + 3)$. A factorisation is complete only when the terms left inside the bracket share no common factor.",
    ],
    latexBlocks: [
      "3x + 12 = 3\\cdot x + 3\\cdot 4 = 3(x + 4) \\quad (\\text{distributive law, reversed})",
      "\\text{HCF}(6, 10) = 2 \\;\\Rightarrow\\; 6x + 10 = 2(3x + 5)",
      "\\text{HCF}(6x, 9x^2) = 3x \\;\\Rightarrow\\; 6x + 9x^2 = 3x(2 + 3x)",
      "\\text{Check by expanding:} \\quad 2(3x+5) = 6x+10 \\;\\checkmark",
    ],
  },
  workedExamples: [
    {
      ...
      steps: [
        { explanation: "Find the largest number dividing both coefficients; list factors if unsure.", latex: "\\text{factors of }12: 1,2,3,4,6,12;\\;\\; 8: 1,2,4,8 \\;\\Rightarrow\\; \\text{HCF}=4" },
        { explanation: "Divide each term by the HCF to see what is left inside the bracket.", latex: "12x \\div 4 = 3x, \\quad 8 \\div 4 = 2" },
        { explanation: "Write the HCF outside and the quotients inside.", latex: "12x + 8 = 4(3x + 2)" },
        { explanation: "Check by expanding back to the original.", latex: "4(3x+2) = 12x + 8 \\;\\checkmark" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Find the HCF of the coefficients first.", latex: "\\text{HCF}(6, 9) = 3" },
        { explanation: "Both terms contain an x, so an x joins the common factor.", latex: "\\text{HCF} = 3x" },
        { explanation: "Divide each term by 3x to find the bracket contents.", latex: "6x \\div 3x = 2, \\quad 9x^2 \\div 3x = 3x" },
        { explanation: "Write the factorised form, then mentally expand to confirm.", latex: "6x + 9x^2 = 3x(2 + 3x)" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Find the largest number dividing all three coefficients.", latex: "\\text{HCF}(10, 15, 5) = 5" },
        { explanation: "Divide each term by 5; note the last term leaves 1, not nothing.", latex: "10a \\div 5 = 2a, \\quad 15b \\div 5 = 3b, \\quad 5 \\div 5 = 1" },
        { explanation: "Write the factorised form, keeping the 1 inside.", latex: "10a + 15b + 5 = 5(2a + 3b + 1)" },
      ],
      finalAnswerLatex: "5(2a + 3b + 1)",
    },
    {
      title: "Factorise fully, then check the bracket is finished (harder)",
      questionLatex: "\\text{Factorise }\\; 24x^2 + 16x \\text{ fully.}",
      steps: [
        { explanation: "Find the HCF of the coefficients.", latex: "\\text{HCF}(24, 16) = 8" },
        { explanation: "An x appears in both terms, so it joins the common factor.", latex: "\\text{HCF} = 8x" },
        { explanation: "Divide each term by 8x.", latex: "24x^2 \\div 8x = 3x, \\quad 16x \\div 8x = 2" },
        { explanation: "Write the factorised form.", latex: "24x^2 + 16x = 8x(3x + 2)" },
        { explanation: "Confirm it is fully factorised: 3x and 2 share no common factor, so it is finished.", latex: "8x(3x+2) = 24x^2 + 16x \\;\\checkmark" },
      ],
      finalAnswerLatex: "8x(3x + 2)",
    },
  ],
  guidedPractice: [
    choice(
...
const substitution: LessonContent = {
  ...
  teaching: {
    paragraphs: [
      "Substitution means putting a specific number in place of a pronumeral, so a general expression turns into one particular calculation. The pronumeral was always a placeholder for 'some number'; substitution is the moment we finally say which number it is.",
      "Take a concrete case. The rule $3x + 1$ describes a whole family of values — one for each $x$. If we are told $x = 4$, we swap the $x$ for 4 wherever it appears, giving $3 \\times 4 + 1$. Every letter is now gone, so it is just arithmetic: $12 + 1 = 13$. Substitution is exactly this swap-then-calculate move, and it works because the letter never meant anything more than 'put a number here'.",
      "Because the result is ordinary arithmetic, the ordinary order of operations still rules it: brackets first, then powers, then multiply and divide, then add and subtract. This matters most with powers. To evaluate $2x^2 + 3$ at $x = 5$, the power binds tighter than the multiplication, so you square first: $5^2 = 25$, then $2 \\times 25 = 50$, then $+3 = 53$. The notation $2x^2$ means $2 \\times (x \\times x)$, not $(2x)^2$ — a distinction that decides the answer.",
      "Negative values are the other place to slow down. A pronumeral can stand for a negative number, and the safe habit is to wrap it in brackets before calculating. At $x = -4$, the expression $3x^2$ becomes $3 \\times (-4)^2 = 3 \\times 16 = 48$: the brackets remind you to square the whole $-4$, and a negative squared is positive. Without the brackets it is easy to slip and write $3 \\times -4 = -12$, squaring the wrong thing.",
      "Formulas are simply named expressions, so they are evaluated the same way. The area of a rectangle is $A = lw$, where writing $l$ and $w$ next to each other means multiply. With $l = 8$ and $w = 5$, substitution gives $A = 8 \\times 5 = 40$. The formula states the general rule; substitution turns it into the one answer you need. The trap is to read $lw$ as 'add' — but two letters side by side always mean a product.",
      "Sometimes the situation is reversed: you are given the answer and must find the input. If $2n + 5 = 13$, you can test values until one fits, but it is faster to undo the operations in reverse. The expression does $\\times 2$ then $+5$ to $n$; to get back to $n$ you peel those off in the opposite order — first subtract 5 ($2n = 8$), then divide by 2 ($n = 4$). Undoing in reverse order is the key idea, and it is exactly why you do not divide before subtracting here.",
      "Whichever direction you work, finish by checking. Substitute your value back into the original: $2(4) + 5 = 13$ matches, so $n = 4$ is correct. This same check confirms a forward calculation too, and it is the habit that turns a plausible answer into a certain one.",
    ],
    latexBlocks: [
      "\\text{If } x = 4: \\quad 3x + 1 = 3 \\times 4 + 1 = 13",
      "2x^2 \\text{ at } x = 5: \\;\\; 2 \\times 5^2 = 2 \\times 25 = 50 \\quad (\\text{power before } \\times)",
      "A = lw \\;\\; (lw \\text{ means } l \\times w); \\quad l=8,\\,w=5 \\Rightarrow A = 40",
      "2n + 5 = 13 \\;\\Rightarrow\\; 2n = 8 \\;\\Rightarrow\\; n = 4 \\quad (\\text{undo in reverse})",
    ],
  },
  workedExamples: [
    {
      ...
      steps: [
        { explanation: "Swap the pronumeral x for the given number 5.", latex: "4 \\times 5 - 3" },
        { explanation: "Order of operations: do the multiplication before the subtraction.", latex: "4 \\times 5 = 20" },
        { explanation: "Now subtract to finish.", latex: "20 - 3 = 17" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "Replace each pronumeral in the formula with its given value.", latex: "P = 2 \\times 9 + 2 \\times 4" },
        { explanation: "Carry out both multiplications before adding.", latex: "P = 18 + 8" },
        { explanation: "Add the results for the perimeter.", latex: "P = 26" },
      ],
    ...
    },
    {
      ...
      steps: [
        { explanation: "The expression was built by times 2 then plus 5; undo the plus 5 first by subtracting from both sides.", latex: "2n = 13 - 5 = 8" },
        { explanation: "Now undo the times 2 by dividing both sides by 2.", latex: "n = 8 \\div 2 = 4" },
        { explanation: "Check by substituting n = 4 back into the original.", latex: "2(4) + 5 = 13 \\;\\checkmark" },
      ],
      finalAnswerLatex: "n = 4",
    },
    {
      title: "Substitute a negative value with a power (harder)",
      questionLatex: "\\text{Find the value of } 2a^2 - 3a \\text{ when } a = -4.",
      steps: [
        { explanation: "Substitute, wrapping the negative value in brackets to keep it whole.", latex: "2 \\times (-4)^2 - 3 \\times (-4)" },
        { explanation: "Powers first: a negative number squared is positive.", latex: "(-4)^2 = 16" },
        { explanation: "Do the two multiplications next.", latex: "2 \\times 16 = 32, \\quad 3 \\times (-4) = -12" },
        { explanation: "Subtracting a negative is adding, so combine carefully.", latex: "32 - (-12) = 32 + 12 = 44" },
      ],
      finalAnswerLatex: "2a^2 - 3a = 44 \\text{ when } a = -4",
    },
  ],
  guidedPractice: [
    choice(
