...
function ans(id: string, prompt: string, latex: string, answer: string, difficulty: number, explanation: string, accepted: string[] = []): PracticeQuestion {
  return { id, prompt, latex, answer, acceptedAnswers: Array.from(new Set([answer, ...accepted])), difficulty, hint: "Think about what the network represents, then count or compare.", explanation };
}
function mcq(id: string, prompt: string, answer: "A" | "B" | "C" | "D", choices: [string, string, string, string], difficulty: number, explanation: string): PracticeQuestion {
  return { id, prompt, latex: "\\text{Select A, B, C, or D.}", choices: ["A", "B", "C", "D"].map((label, i) => ({ label, text: choices[i] })), answer, difficulty, hint: "Reason about the network structure, then check each option.", explanation };
}
...
const introNetworks: Partial<ExplicitLesson> = {
  ...
  guidedPractice: [
    ...
    ans("y10-itn-g3", "A vertex has 4 edges meeting it. State its degree.", "\\deg(v)", "4", 2, "Degree = number of edges at the vertex = 4.", []),
  ...
  independentPractice: [
    ans("y10-itn-i1", "A network has 6 edges. Find the sum of all the vertex degrees.", "6 \\text{ edges}", "12", 2, "Sum of degrees = 2 × 6 = 12.", []),
    ...
    ans("y10-itn-i3", "In a triangle network (3 vertices, each joined to the other two), state the degree of each vertex.", "\\text{triangle}", "2", 3, "Each vertex joins the other two, so degree 2.", []),
  ...
  masteryQuiz: [
    ...
    ans("y10-itn-m8", "In a square network (4 vertices in a cycle), state the degree of each vertex.", "\\text{4-cycle}", "2", 3, "Each vertex joins its two neighbours, so degree 2.", []),
    ...
    ans("y10-itn-m10", "A vertex joined to all 3 others in a 4-vertex network has what degree?", "\\text{joined to 3}", "3", 2, "It meets 3 edges, so degree 3.", []),
  ],
  commonMistakes: [
...
const isomorphicPlanar: Partial<ExplicitLesson> = {
  ...
  independentPractice: [
    ...
    ans("y10-ipg-i2", "A connected planar graph has V = 6, E = 9. Find F.", "V=6,E=9", "5", 3, "F = 2 − 6 + 9 = 5.", []),
  ...
  masteryQuiz: [
    ...
    ans("y10-ipg-m5", "A connected planar graph has V = 5, E = 6. Find F.", "V=5,E=6", "3", 3, "F = 2 − 5 + 6 = 3.", []),
...
const trailsPathsEulerian: Partial<ExplicitLesson> = {
  ...
  independentPractice: [
    ...
    ans("y10-tpe-i5", "A graph has exactly 2 odd-degree vertices. Does an Eulerian trail exist? (yes/no)", "2 \\text{ odd}", "yes", 4, "0 or 2 odd-degree vertices means an Eulerian trail exists.", ["Yes"]),
  ],
  masteryQuiz: [
    ...
    ans("y10-tpe-m4", "A graph has 4 odd-degree vertices. Does an Eulerian trail exist? (yes/no)", "4 \\text{ odd}", "no", 4, "More than 2 odd-degree vertices means no Eulerian trail.", ["No"]),
    ...
    ans("y10-tpe-m9", "A graph has 0 odd-degree vertices. Does an Eulerian circuit exist? (yes/no)", "0 \\text{ odd}", "yes", 3, "All-even degrees give an Eulerian circuit.", ["Yes"]),
...
const shortestPath: Partial<ExplicitLesson> = {
  ...
  guidedPractice: [
    ...
    ans("y10-spp-g3", "Route A totals 5 + 4 = 9; route B totals 10. Give the shortest total weight.", "A:9, B:10", "9", 3, "9 < 10, so route A is shorter.", []),
  ...
  independentPractice: [
    ...
    ans("y10-spp-i2", "Route A: 3 + 3 + 3. Route B: 4 + 6. Give the shortest total weight.", "A:9, B:10", "9", 4, "A = 9, B = 10; shortest is 9 (even though A has more edges).", []),
  ...
  masteryQuiz: [
    ...
    ans("y10-spp-m2", "Two routes have total weights 7 and 5. Give the shortest.", "7 \\text{ vs } 5", "5", 3, "5 < 7.", []),
    ...
    ans("y10-spp-m7", "Three routes total 12, 9 and 15. Give the shortest.", "12,9,15", "9", 3, "9 is the least total weight.", []),
