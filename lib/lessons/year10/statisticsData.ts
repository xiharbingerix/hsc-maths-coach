...
const quartilesMastery: PracticeQuestion[] = [
  ...
  statAnswer("y10-stat-iqr-m2", "Find the lower quartile of the ordered dataset.", "4,\\ 7,\\ 9,\\ 12,\\ 15,\\ 18,\\ 21", "7", "The lower half is {4, 7, 9} and its median is 7."),
  statAnswer("y10-stat-iqr-m3", "Find the interquartile range of the ordered dataset.", "2,\\ 5,\\ 8,\\ 11,\\ 14,\\ 17,\\ 20", "12", "Q1 is the median of {2, 5, 8} = 5 and Q3 is the median of {14, 17, 20} = 17. IQR = 17 − 5 = 12."),
  statAnswer("y10-stat-iqr-m4", "The upper quartile is 32 and the interquartile range is 14. Find the lower quartile.", "Q_3=32,\\quad IQR=14", "18", "Subtract the IQR from the upper quartile: Q1 = 32 − 14 = 18."),
  statChoice("y10-stat-iqr-m5", "Two datasets both have a range of 40. Dataset A has interquartile range 8; Dataset B has interquartile range 30. Which conclusion is valid?", "B", ["They have the same spread because their ranges are equal", "Dataset A's middle half is more tightly clustered than Dataset B's", "Dataset B must have the smaller median", "IQR cannot be compared when the ranges are equal"], "Equal ranges only mean the extreme values span the same distance. The IQR measures the middle 50%: A's IQR of 8 is far smaller than B's 30, so A's middle half is more tightly clustered. Range alone hides this difference."),
...
const sdMastery: PracticeQuestion[] = [
  ...
  statChoice("y10-stat-sd-m5", "Dataset A has mean 50 and standard deviation 4. Dataset B has mean 5 and standard deviation 4. Which statement is correct?", "A", ["Both datasets have values equally spread around their own mean", "Dataset A is more spread out because its mean is larger", "Dataset B is more spread out because its values are smaller", "Standard deviations cannot be compared across different means"], "Standard deviation measures spread around the mean, independent of how large the mean is. Both datasets have standard deviation 4, so their values cluster equally tightly around their respective means."),
  ...
  statAnswer("y10-stat-sd-m7", "Find the population standard deviation of the dataset.", "2,\\ 2,\\ 10,\\ 10", "4", "The mean is 6 and each value is four units from the mean, so the standard deviation is 4."),
...
const bestFitMastery: PracticeQuestion[] = [
  ...
  statChoice("y10-stat-fit-m4", "A line of best fit y = 2x + 7 was built from data with inputs ranging from 1 to 10. Which statement about predicting the output at x = 20 is correct?", "B", ["y = 47, and it is reliable because the line is straight", "y = 47, but it is an extrapolation so should be treated with caution", "y = 27, an interpolation within the data", "The output cannot be calculated"], "Substituting gives y = 2(20) + 7 = 47, but x = 20 lies outside the observed input range of 1 to 10, so this is an extrapolation. The trend may not continue that far, making the prediction less reliable.", "y=2x+7,\\quad \\text{inputs }1\\text{ to }10"),
...
export function year10StatisticsDataLessonOverride(
  ...
  if (!["year-10-mathematics", "year-10-mathematics-advanced", "year-10-mathematics-core"].includes(course.slug) || !["single-variable-bivariate-statistics"].includes(unit.slug)) return null;
