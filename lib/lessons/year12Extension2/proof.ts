...
const contradictionLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...proofChoice("y12e2-proof-contr-pool-1", "A proof by contradiction begins by assuming which of the following?", "B", ["The statement to be proved.", "The negation of the statement to be proved.", "A single numerical example.", "The converse of the statement."], "A proof by contradiction starts by assuming the negation of the target statement, then derives an impossibility."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-contr-pool-2", "In proving √2 is irrational you assume √2 = p/q in lowest terms and square to get p² = 2q². State whether p is even or odd.", "p^2 = 2q^2", "even", ["Even", "EVEN"], "p² = 2q² is even, and a perfect square is even only when its base is even, so p is even."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-contr-pool-3", "Since p is even, write p = 2r and substitute into p² = 2q². This gives q² = c·r². Find c.", "(2r)^2 = 2q^2 \\Rightarrow q^2 = c\\,r^2", "2", [], "4r² = 2q² ⟹ q² = 2r², so c = 2 — which makes q even too, the contradiction with lowest terms."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-contr-pool-4", "Begin the same proof for √5: assume √5 = p/q (lowest terms) and square. The equation p² = k·q² has what value of k?", "p^2 = k\\,q^2", "5", [], "Squaring √5 = p/q gives 5 = p²/q², so p² = 5q² and k = 5."), difficulty: 3 },
    { ...proofChoice("y12e2-proof-contr-pool-5", "A student assumes √2 = p/q but does not require the fraction to be in lowest terms. Why does the proof fail to reach a contradiction?", "C", ["Squaring both sides is invalid.", "p and q must both be prime.", "Finding a shared factor only contradicts the assumption if the fraction was taken in lowest terms.", "√2 cannot be squared."], "Without the lowest-terms assumption, discovering that p and q share a factor of 2 is not a contradiction — it is only a contradiction when gcd(p, q) was assumed to be 1."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-contr-pool-6", "Begin the proof for √7: assume √7 = p/q (lowest terms). Since p² = 7q², 7 divides p, so p = 7r. Substitute to get q² = k·r². Find k.", "(7r)^2 = 7q^2 \\Rightarrow q^2 = k\\,r^2", "7", [], "49r² = 7q² ⟹ q² = 7r², so k = 7; then 7 divides q as well, contradicting lowest terms."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contr-pool-7", "Show there are no integers x, y with 6x + 9y = 5. The left-hand side is always divisible by which integer greater than 1?", "6x + 9y = 3(2x + 3y)", "3", [], "6x + 9y = 3(2x + 3y) is always a multiple of 3, but 5 is not divisible by 3, so no integer solutions exist."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contr-pool-8", "Assume √2 + √3 is rational, equal to s. Squaring gives 5 + 2√6 = s², so √6 = (s² − 5)/2, which would be rational. This contradicts the fact that √6 is ___ (one word).", "(\\sqrt2+\\sqrt3)^2 = 5 + 2\\sqrt6", "irrational", ["Irrational"], "√6 is irrational, so it cannot equal the rational number (s² − 5)/2 — the contradiction."), difficulty: 4 },
    { ...proofChoice("y12e2-proof-contr-pool-9", "A contradiction proof derives a surprising but not impossible statement and stops, claiming a contradiction. What is missing?", "D", ["The proof should test more numerical cases.", "The assumption should be the original statement.", "Algebra is not allowed in a contradiction proof.", "The derived statement must conflict with a known fact or earlier condition."], "A genuine contradiction must conflict with something already established (a known fact or an earlier condition); merely surprising is not enough."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contr-pool-10", "Assume log₂5 = p/q with positive integers p and q. Rearranging gives the equation shown — the left side is a power of 2 (even) and the right side a power of 5 (odd). How many positive-integer solutions (p, q) are there?", "2^p = 5^q", "0", ["none"], "An even number (2 to a positive power) can never equal an odd number (5 to a positive power), so there are 0 solutions — hence log₂5 is irrational."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-contr-pool-11", "To prove 'if n² is even then n is even' by contradiction, assume n is odd, n = 2k + 1. Find the remainder when n² is divided by 2.", "n = 2k+1 \\Rightarrow n^2 = 4k^2 + 4k + 1", "1", [], "n² = 4k² + 4k + 1 = 2(2k² + 2k) + 1, remainder 1 (odd). This contradicts n² being even, so n must be even."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-contr-pool-12", "For a prime P, assume √P = p/q in lowest terms. The argument forces P to divide both p and q. The gcd of p and q was assumed to be 1, but it must now be at least what value?", "\\sqrt{P} = p/q,\\ \\gcd(p,q)=1", "P", [], "If P divides both p and q then gcd(p, q) ≥ P > 1, contradicting the lowest-terms assumption gcd(p, q) = 1 — the general prime case."), difficulty: 5 },
  ],
};
...
const contrapositiveLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...proofChoice("y12e2-proof-contra-pool-1", "The contrapositive of 'if P then Q' is which statement?", "C", ["If Q then P (the converse).", "If not P then not Q (the inverse).", "If not Q then not P.", "P if and only if Q (the biconditional)."], "The contrapositive negates AND swaps both parts: 'if not Q then not P'. It is logically equivalent to the original."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-contra-pool-2", "The contrapositive of 'if n is odd then n² is odd' is 'if n² is ___ then n is even'. Fill the blank (even or odd).", "\\text{if } n^2 \\text{ even then } n \\text{ even}", "even", ["Even"], "Negate and swap: not(n² odd) = n² even, not(n odd) = n even. So 'if n² is even then n is even'."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-contra-pool-3", "The contrapositive of 'if n² is even then n is even' is 'if n is odd then n² is ___'. Fill the blank.", "\\text{if } n \\text{ odd then } n^2 \\text{ odd}", "odd", ["Odd"], "Negate and swap: not(n even) = n odd, not(n² even) = n² odd."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-contra-pool-4", "Proving 'if n² is divisible by 3 then n is divisible by 3' by contrapositive: assume 3 does not divide n, so n = 3m+1 or 3m+2. For n = 3m+1, find the remainder when n² is divided by 3.", "(3m+1)^2 = 9m^2 + 6m + 1", "1", [], "(3m+1)² = 9m² + 6m + 1 = 3(3m² + 2m) + 1, remainder 1 — so n² is not divisible by 3."), difficulty: 3 },
    { ...proofChoice("y12e2-proof-contra-pool-5", "To prove 'if P then Q', a student proves 'if Q then P'. What have they actually proved?", "B", ["The contrapositive.", "The converse, which is not logically equivalent to the original.", "The inverse.", "The original statement directly."], "Swapping P and Q gives the converse, which is generally NOT equivalent to the original — a common error mistaken for the contrapositive."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-contra-pool-6", "Prove 'if n³ is even then n is even' by contrapositive: assume n is odd, n = 2k+1. State whether n³ is even or odd.", "n = 2k+1", "odd", ["Odd"], "An odd number cubed is odd: (2k+1)³ is odd. So 'n odd ⟹ n³ odd', the contrapositive, proves the original."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contra-pool-7", "The contrapositive of 'if x > 0 then x³ > 0' is 'if x³ ≤ 0 then x ≤ 0'. The boundary value at which x³ = 0 is x = ___.", "x^3 = 0", "0", [], "x³ = 0 only at x = 0 — the boundary separating the positive and non-positive cases the contrapositive splits on."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contra-pool-8", "Contrapositive of 'if n is not divisible by 5 then n² is not divisible by 5': for n = 5m+2, find the remainder when n² is divided by 5.", "(5m+2)^2 = 25m^2 + 20m + 4", "4", [], "(5m+2)² = 25m² + 20m + 4 = 5(5m² + 4m) + 4, remainder 4 — so n² is not divisible by 5."), difficulty: 4 },
    { ...proofChoice("y12e2-proof-contra-pool-9", "Why does proving the contrapositive prove the original statement?", "C", ["Because the converse is always true.", "Because negating a statement twice returns it.", "Because a conditional and its contrapositive are false in exactly the same case (P true, Q false).", "Because every implication equals its converse."], "A conditional 'if P then Q' is false only when P is true and Q is false. Its contrapositive is false in exactly that same case, so the two always share their truth value."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-contra-pool-10", "The contrapositive method proves 'if n² is divisible by 3 then n is divisible by 3', but the same claim with 4 is FALSE. Test n = 2: find the remainder when n² is divided by 4.", "2^2 = 4", "0", [], "2² = 4 has remainder 0, so 4 divides n² while 4 does not divide n = 2. The method fails for 4 because 4 is not prime — a prime is what makes the contrapositive valid."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-contra-pool-11", "Generalising to a prime P: assume P does not divide n, so n ≡ r (mod P) with 1 ≤ r ≤ P−1. Then n² ≡ r² (mod P) is nonzero. Name the one-word property of P that this step relies on.", "n \\equiv r \\pmod P,\\ 1 \\le r \\le P-1", "prime", ["primality", "Prime"], "If P is prime and 1 ≤ r ≤ P−1, then P cannot divide r² (a prime divides a product only if it divides a factor). Primality is exactly what keeps r² nonzero mod P."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-contra-pool-12", "The contrapositive of 'if a + b is odd then a and b have different parity' is 'if a and b have the SAME parity then a + b is ___'. Fill the blank.", "a, b \\text{ same parity}", "even", ["Even"], "Same parity: even + even = even and odd + odd = even. So a + b is even — the contrapositive of the original statement."), difficulty: 5 },
  ],
};
...
const inequalitiesLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...proofChoice("y12e2-proof-ineq-pool-1", "To prove A ≥ B for all real values, the standard algebraic method is to show that A − B is always which of the following?", "B", ["Negative.", "Non-negative — for example a square, or a square plus a positive constant.", "Exactly zero.", "Irrational."], "Proving A − B ≥ 0 proves A ≥ B. The usual route is to rewrite A − B as a visibly non-negative expression such as a square or a sum of squares."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-ineq-pool-2", "Complete the square: x² + 6x + 11 = (x + 3)² + c. Find c.", "x^2 + 6x + 11 = (x+3)^2 + c", "2", [], "(x + 3)² = x² + 6x + 9, so x² + 6x + 11 = (x + 3)² + 2 and c = 2."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-ineq-pool-3", "Using the completed square x² + 6x + 11 = (x + 3)² + 2, state the minimum value of x² + 6x + 11.", "(x+3)^2 + 2", "2", [], "(x + 3)² ≥ 0, so the expression is at least 2, achieved at x = −3. Minimum value 2 (and 2 > 0, proving positivity)."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-ineq-pool-4", "Complete the square: x² − 4x + 9 = (x − 2)² + c. Find c.", "x^2 - 4x + 9 = (x-2)^2 + c", "5", [], "(x − 2)² = x² − 4x + 4, so x² − 4x + 9 = (x − 2)² + 5 and c = 5."), difficulty: 3 },
    { ...proofChoice("y12e2-proof-ineq-pool-5", "A student proves a² + b² ≥ 2ab by writing (a + b)² ≥ 0 and expanding. Why is this invalid?", "C", ["Squares can be negative.", "You cannot expand (a + b)².", "(a + b)² ≥ 0 gives a² + b² ≥ −2ab, not ≥ 2ab; the correct square is (a − b)².", "The inequality only holds for positive numbers."], "Expanding (a + b)² ≥ 0 yields a² + 2ab + b² ≥ 0, i.e. a² + b² ≥ −2ab. To get a² + b² ≥ 2ab you must use (a − b)² ≥ 0."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-ineq-pool-6", "Prove a² + b² ≥ 2ab using (a − b)² ≥ 0. State the value of a − b at which equality holds (take a = b = 5).", "(a-b)^2 \\ge 0", "0", [], "Equality in (a − b)² ≥ 0 occurs when a − b = 0, i.e. a = b. With a = b = 5, a − b = 0."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-ineq-pool-7", "For positive reals a, b with a + b = 10, the AM–GM inequality gives an upper bound for ab. Find the maximum value of ab.", "ab \\le \\left(\\tfrac{a+b}{2}\\right)^2", "25", [], "AM–GM: ab ≤ ((a + b)/2)² = (10/2)² = 25, with equality at a = b = 5. Maximum ab = 25."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-ineq-pool-8", "The quadratic x² + kx + 9 is positive for all real x. The boundary value of k (where the discriminant is zero, taking k > 0) is ___.", "k^2 - 36 = 0", "6", [], "Positive for all x needs discriminant k² − 36 < 0. The boundary k² − 36 = 0 gives k = 6 (positive value)."), difficulty: 4 },
    { ...proofChoice("y12e2-proof-ineq-pool-9", "Which is a valid proof that x² − 6x + 11 > 0 for all real x?", "C", ["Test x = 0, 1, 2 and observe the result is positive.", "Since x² > 0, the whole expression is positive.", "x² − 6x + 11 = (x − 3)² + 2, and a square is ≥ 0 with 2 > 0, so the sum is > 0.", "The leading coefficient is positive, so it is always positive."], "Completing the square gives (x − 3)² + 2. A square is non-negative and 2 > 0, so the expression is strictly positive for every x. Testing values or citing the leading coefficient alone does not prove it for all x."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-ineq-pool-10", "For a > 0, the AM–GM inequality gives a + 1/a ≥ k. Find the minimum value k (the value at equality).", "a + \\tfrac{1}{a} \\ge 2\\sqrt{a \\cdot \\tfrac{1}{a}}", "2", [], "By AM–GM, a + 1/a ≥ 2√(a · 1/a) = 2, with equality at a = 1. Minimum value 2."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-ineq-pool-11", "To prove a² + b² + c² ≥ ab + bc + ca, doubling and rearranging gives (a − b)² + (b − c)² + (c − a)² ≥ 0. At equality (a = b = c), find the value of (a − b)² + (b − c)² + (c − a)².", "(a-b)^2 + (b-c)^2 + (c-a)^2 \\ge 0", "0", [], "2(a² + b² + c² − ab − bc − ca) = (a − b)² + (b − c)² + (c − a)² ≥ 0. Equality needs each square zero, i.e. a = b = c, giving the sum 0."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-ineq-pool-12", "Prove x⁴ − x² + 1 > 0 for all real x by writing it as (x² − 1/2)² + c. Find c (the fact that c > 0 completes the proof).", "x^4 - x^2 + 1 = (x^2 - \\tfrac12)^2 + c", "3/4", ["0.75"], "(x² − 1/2)² = x⁴ − x² + 1/4, so x⁴ − x² + 1 = (x² − 1/2)² + 3/4. Since a square is ≥ 0 and 3/4 > 0, the expression is always positive; c = 3/4."), difficulty: 5 },
  ],
};
...
const inductionLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...proofChoice("y12e2-proof-induct-pool-1", "A proof by induction has three steps: prove the base case, assume the statement holds for n = k, and then do what?", "B", ["Assume the statement holds for n = k + 1.", "Prove the statement for n = k + 1 follows from the n = k case.", "Check the statement for n = k + 2.", "Verify three numerical cases."], "The inductive step must DERIVE P(k + 1) from the assumed P(k) — not assume P(k + 1)."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-induct-pool-2", "For the claim '3 divides 4ⁿ − 1', evaluate the base case by computing 4¹ − 1.", "4^1 - 1", "3", [], "4¹ − 1 = 3, which is divisible by 3, so the base case holds."), difficulty: 2 },
    { ...proofTyped("y12e2-proof-induct-pool-3", "For 1 + 2 + … + n = n(n+1)/2, evaluate the base case at n = 1 (both sides should agree). State that common value.", "\\tfrac{1\\cdot 2}{2}", "1", [], "LHS = 1 and RHS = 1·2/2 = 1, so both sides equal 1 — the base case holds."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-induct-pool-4", "For the inequality claim 2ⁿ > n, evaluate the base case at n = 1: compute LHS − RHS.", "2^1 - 1", "1", [], "At n = 1, LHS = 2 and RHS = 1, so LHS − RHS = 1 > 0; the base case holds."), difficulty: 3 },
    { ...proofChoice("y12e2-proof-induct-pool-5", "A student verifies the base case, then assumes the statement for n = k + 1 and derives n = k + 2. Why is this invalid?", "C", ["The base case is unnecessary.", "Induction cannot be used for inequalities.", "They assumed the very thing to be proved; the step must derive P(k + 1) FROM the assumed P(k).", "Induction only proves a single value."], "Assuming P(k + 1) is circular. A valid inductive step assumes P(k) and proves P(k + 1) follows from it."), difficulty: 3 },
    { ...proofTyped("y12e2-proof-induct-pool-6", "Inductive step for '3 divides 4ⁿ − 1': the n = k+1 case 4ᵏ⁺¹ − 1 can be written as 4(4ᵏ − 1) + c. Find c.", "4^{k+1} - 1 = 4(4^{k} - 1) + c", "3", [], "Since 4ᵏ⁺¹ = 4·4ᵏ, we have 4ᵏ⁺¹ − 1 = 4(4ᵏ − 1) + 3, so c = 3. The hypothesis makes 4ᵏ − 1 divisible by 3, and 3 is divisible by 3, so 4ᵏ⁺¹ − 1 is divisible by 3."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-induct-pool-7", "Inductive step for the sum 1 + 2 + … + n = n(n+1)/2: assuming the k case, the (k+1)-th partial sum is k(k+1)/2 + (k+1), which factors as (k+1)(k+2)/d. Find d.", "\\tfrac{k(k+1)}{2} + (k+1) = \\tfrac{(k+1)(k+2)}{d}", "2", [], "k(k+1)/2 + (k+1) = (k+1)(k/2 + 1) = (k+1)(k+2)/2, so d = 2 — matching the formula with n = k+1."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-induct-pool-8", "The claim 'n² + an is divisible by 2 for all n ≥ 1' needs the base case n = 1, i.e. 1 + a divisible by 2. Find the smallest positive integer a.", "1 + a \\equiv 0 \\pmod 2", "1", [], "1 + a must be even, so a must be odd. The smallest positive odd integer is a = 1."), difficulty: 4 },
    { ...proofChoice("y12e2-proof-induct-pool-9", "Why does verifying P(1), P(2) and P(3) NOT prove that P(n) holds for all n ≥ 1?", "D", ["Because P(4) is always false.", "Because the base case should be P(0).", "Because induction needs at least five cases.", "Because checking finitely many cases never establishes the general statement; the inductive step is what extends it to all n."], "No finite number of checks proves a statement for all integers. The k → k+1 step is exactly what carries it through the whole domain."), difficulty: 4 },
    { ...proofTyped("y12e2-proof-induct-pool-10", "For Σ k³ = [n(n+1)/2]² the inductive step adds (k+1)³ to [k(k+1)/2]² and must give [(k+1)(k+2)/2]². Verify the formula directly at n = 2 by computing 1³ + 2³.", "\\sum_{k=1}^{2} k^3", "9", [], "1³ + 2³ = 1 + 8 = 9, and [2·3/2]² = 3² = 9 — the two agree, confirming the step's target form."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-induct-pool-11", "Inductive divisibility: n³ − n = (n−1)n(n+1) is a product of three consecutive integers, so 6 divides it. Evaluate n³ − n at n = 2.", "n^3 - n = (n-1)n(n+1)", "6", [], "At n = 2, n³ − n = 8 − 2 = 6 = 1·2·3, divisible by 6 — consistent with the general result."), difficulty: 5 },
    { ...proofTyped("y12e2-proof-induct-pool-12", "The inequality 2ⁿ ≥ n² is proved by induction starting at n = 4, not n = 1. Find the largest integer n < 4 for which 2ⁿ ≥ n² FAILS (this is why the base case is 4).", "2^n \\ge n^2", "3", [], "n = 1: 2 ≥ 1 ✓; n = 2: 4 ≥ 4 ✓; n = 3: 8 ≥ 9 ✗. It fails at n = 3, so induction must start from n = 4."), difficulty: 5 },
  ],
};
