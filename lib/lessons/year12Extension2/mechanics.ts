...
const circularMotionLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    ...
    { ...mechTyped("y12e2-mech-circ-pool-11", "On a frictionless banked track the design speed satisfies v² = rg·tanθ. A track is banked at a FIXED angle θ. If its radius r is doubled (θ unchanged), by what factor does the design speed v change?", "v^2 = rg\\tan\\theta,\\ \\theta\\text{ fixed}", "√2", ["sqrt2", "root 2", "1.41", "1.414"], "With θ fixed, v² ∝ r, so v ∝ √r. Doubling r doubles v², multiplying v by √2 ≈ 1.41 — the safe speed grows with the SQUARE ROOT of the radius, not linearly. (Recognising the proportionality, not plugging numbers, is the point.)"), difficulty: 5 },
...
const projectileMotionResistanceLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...mechChoice("y12e2-mech-proj-pool-1", "Without air resistance the horizontal velocity of a projectile is constant. With LINEAR air resistance, the horizontal velocity instead:", "B", ["Stays constant.", "Decreases exponentially toward zero.", "Increases without bound.", "Becomes negative."], "Horizontal resistance gives ẍ = −kẋ, so ẋ = ẋ₀·exp(−kt) — the horizontal speed decays exponentially rather than staying constant."), difficulty: 2 },
    { ...mechTyped("y12e2-mech-proj-pool-2", "Horizontal motion with resistance: vx = vx0·exp(−kt). For vx0 = 20 m/s and k = ln2 per second, find vx at t = 1 s.", "v_x = v_{x0} e^{-kt}", "10", ["10 m/s"], "vx = 20·exp(−ln2·1) = 20·(1/2) = 10 m/s."), difficulty: 2 },
    { ...mechTyped("y12e2-mech-proj-pool-3", "The horizontal displacement is x = (vx0/k)(1 − exp(−kt)); as t → ∞ it approaches the limit vx0/k. For vx0 = 20 m/s and k = 2 per second, find this limiting horizontal distance.", "x \\to \\frac{v_{x0}}{k}", "10", ["10 m"], "As t → ∞, exp(−kt) → 0, so x → vx0/k = 20/2 = 10 m."), difficulty: 3 },
    { ...mechTyped("y12e2-mech-proj-pool-4", "For vx = vx0·exp(−kt), the time for the horizontal speed to halve satisfies exp(−kt) = 1/2, i.e. kt = ln2. For k = ln2 per second, find that time t.", "e^{-kt} = \\tfrac12", "1", ["1 s"], "kt = ln2 with k = ln2 gives t = 1 s."), difficulty: 3 },
    { ...mechChoice("y12e2-mech-proj-pool-5", "A student claims air resistance does not change the horizontal range because horizontal velocity is 'always constant'. Why is this wrong?", "C", ["Resistance increases the horizontal speed.", "Horizontal and vertical motions are identical.", "Resistance decelerates the horizontal motion (vx decays exponentially), so the range is LESS than the no-resistance case and is bounded by vx0/k.", "The projectile never lands."], "With resistance the horizontal velocity is not constant — it decays — so the projectile cannot travel as far; its horizontal reach is capped at vx0/k."), difficulty: 3 },
    { ...mechTyped("y12e2-mech-proj-pool-6", "On the way up, vertical motion has a = −(g + k·vy). At the highest point vy = 0, so a = −g. For g = 10 m/s², find the magnitude of the acceleration at the top.", "a = -(g + k v_y)", "10", ["10 m/s²"], "At the top vy = 0, so the resistance term vanishes and a = −g, magnitude 10 m/s²."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-proj-pool-7", "With resistance, the total horizontal distance is bounded by vx0/k no matter how long the flight. For vx0 = 30 m/s and k = 3 per second, find this maximum horizontal distance.", "x_{max} = \\frac{v_{x0}}{k}", "10", ["10 m"], "x_max = vx0/k = 30/3 = 10 m — the horizontal motion can never exceed this bound."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-proj-pool-8", "During the fall, the vertical velocity approaches a terminal value vy_T = g/k. For g = 10 m/s² and k = 4 per second, find vy_T.", "v_{yT} = \\frac{g}{k}", "2.5", ["5/2", "2.5 m/s"], "vy_T = g/k = 10/4 = 2.5 m/s."), difficulty: 4 },
    { ...mechChoice("y12e2-mech-proj-pool-9", "For a projectile launched upward WITH air resistance, the time to rise compared with the time to fall back is:", "C", ["Equal, as in the no-resistance case.", "The rise takes longer than the fall.", "The rise takes LESS time than the fall, because resistance adds to gravity going up but opposes gravity coming down.", "Both are zero."], "Going up, gravity and drag both decelerate the object (fast deceleration → short rise). Coming down, drag opposes gravity (slower → longer fall). The path is asymmetric."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-proj-pool-10", "The limiting horizontal distance is vx0/k. A projectile is launched with horizontal speed 40 m/s and k = 5 per second. Find this limiting horizontal distance.", "x_{max} = \\frac{v_{x0}}{k}", "8", ["8 m"], "x_max = 40/5 = 8 m."), difficulty: 5 },
    { ...mechTyped("y12e2-mech-proj-pool-11", "Horizontal speed decays as vx = vx0·exp(−kt). At time t = 2(ln2)/k, evaluate exp(−kt) by recognising exp(−2 ln2) = 2⁻², then find vx for vx0 = 40 m/s.", "\\exp(-2\\ln 2) = 2^{-2}", "10", ["10 m/s"], "exp(−2 ln2) = (exp(ln2))⁻² = 2⁻² = 1/4, so vx = 40·(1/4) = 10 m/s — the exponential collapses to a power of 2 once you see ln2 in the exponent."), difficulty: 5 },
    { ...mechTyped("y12e2-mech-proj-pool-12", "Explain by result: the horizontal range with resistance is bounded by vx0/k, so a larger k means a shorter reach. Two projectiles both have vx0 = 24 m/s; one has k = 6, the other k = 3. Find the ratio (range limit of the k = 6 case) : (range limit of the k = 3 case), as a single number.", "\\frac{v_{x0}/6}{v_{x0}/3}", "1/2", ["0.5"], "Range limits are 24/6 = 4 m and 24/3 = 8 m, so the ratio is 4 : 8 = 1/2 — doubling k halves the horizontal reach."), difficulty: 5 },
  ],
};
...
const forcesInclinedPlanesLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...mechChoice("y12e2-mech-incline-pool-1", "On a smooth inclined plane at angle θ, the component of gravity directed ALONG (down) the plane is:", "B", ["mg cosθ", "mg sinθ", "mg tanθ", "mg"], "Resolving the weight, the component along the slope is mg sinθ and the component perpendicular to it (the normal direction) is mg cosθ."), difficulty: 2 },
    { ...mechTyped("y12e2-mech-incline-pool-2", "A block of mass m = 2 kg sits on a smooth incline at θ = 30° (g = 10 m/s²). The force pulling it down the slope is mg sinθ. Find this force.", "F = mg\\sin\\theta", "10", ["10 N"], "F = 2·10·sin30° = 2·10·0.5 = 10 N."), difficulty: 2 },
    { ...mechTyped("y12e2-mech-incline-pool-3", "The acceleration of a block sliding down a smooth incline is a = g sinθ. For θ = 30° and g = 10 m/s², find a.", "a = g\\sin\\theta", "5", ["5 m/s²"], "a = 10·sin30° = 10·0.5 = 5 m/s²."), difficulty: 3 },
    { ...mechTyped("y12e2-mech-incline-pool-4", "The normal reaction on an incline is N = mg cosθ. For m = 2 kg, θ = 60°, g = 10 m/s², find N.", "N = mg\\cos\\theta", "10", ["10 N"], "N = 2·10·cos60° = 2·10·0.5 = 10 N."), difficulty: 3 },
    { ...mechChoice("y12e2-mech-incline-pool-5", "A student uses mg cosθ as the force pulling the block down the slope. Why is this wrong?", "C", ["mg cosθ is always zero.", "The force down the slope is mg.", "mg cosθ is the component PERPENDICULAR to the plane (the normal direction); the force along the slope is mg sinθ.", "cosθ and sinθ are equal on an incline."], "Resolving the weight: along the slope is mg sinθ, perpendicular to it is mg cosθ. Swapping them is the classic incline error."), difficulty: 3 },
    { ...mechTyped("y12e2-mech-incline-pool-6", "On a rough incline the friction force is f = μN = μ·mg cosθ. For μ = 0.5, m = 2 kg, θ = 60°, g = 10 m/s², find f.", "f = \\mu mg\\cos\\theta", "5", ["5 N"], "N = 2·10·cos60° = 10 N, so f = 0.5·10 = 5 N."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-incline-pool-7", "A block stays at rest on a rough incline while mg sinθ ≤ μ·mg cosθ, i.e. tanθ ≤ μ. For μ = 1, find the maximum angle θ (in degrees) before it slips.", "\\tan\\theta \\le \\mu", "45", ["45°", "45 deg"], "Slipping begins when tanθ = μ = 1, i.e. θ = 45°."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-incline-pool-8", "The angle of repose θ (the steepest angle with no sliding) satisfies tanθ = μ. For μ = √3, find θ in degrees.", "\\tan\\theta = \\mu", "60", ["60°", "60 deg"], "tanθ = √3 gives θ = 60°."), difficulty: 4 },
    { ...mechChoice("y12e2-mech-incline-pool-9", "Increasing the mass of a block on a rough incline (angle fixed) changes its acceleration down the slope how?", "C", ["It increases proportionally.", "It decreases.", "Not at all — both mg sinθ and friction μmg cosθ scale with m, so a = g(sinθ − μcosθ) is independent of mass.", "It becomes zero."], "Mass cancels: a = g(sinθ − μcosθ). The driving force and friction both grow with m, leaving the acceleration unchanged."), difficulty: 4 },
    { ...mechTyped("y12e2-mech-incline-pool-10", "A block is released from rest on a smooth 30° incline (a = g sinθ = 5 m/s²). Using v² = 2as, find its speed after sliding s = 10 m down the slope.", "v^2 = 2as", "10", ["10 m/s"], "v² = 2·5·10 = 100, so v = 10 m/s."), difficulty: 5 },
    { ...mechTyped("y12e2-mech-incline-pool-11", "A block on a smooth 30° incline is held in equilibrium by a force P applied up the slope, balancing gravity's component: P = mg sinθ. For m = 4 kg, θ = 30°, g = 10 m/s², find P.", "P = mg\\sin\\theta", "20", ["20 N"], "P = 4·10·sin30° = 4·10·0.5 = 20 N."), difficulty: 5 },
    { ...mechTyped("y12e2-mech-incline-pool-12", "Explain by result: the down-slope acceleration a = g sinθ rises from 0 (flat, θ = 0°) to g (vertical, θ = 90°), where the block is in free fall. State a at θ = 90° (g = 10 m/s²).", "a = g\\sin 90°", "10", ["10 m/s²"], "sin90° = 1, so a = g = 10 m/s² — at θ = 90° the incline is vertical and the motion is free fall, as expected."), difficulty: 5 },
  ],
};
