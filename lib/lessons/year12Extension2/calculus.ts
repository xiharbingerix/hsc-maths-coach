...
const partialFractionsQuadraticLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    ...
    { ...calcTyped("y12e2-calc-pfq-pool-12", "Check the decomposition 1/((x+1)(x²+1)) = (1/2)/(x+1) + (−x/2 + 1/2)/(x²+1) by substituting x = 0 into the right-hand side (it should equal the left-hand value 1). State that value.", "\\text{RHS at } x=0", "1", [], "RHS at x = 0: (1/2)/1 + (0 + 1/2)/1 = 1/2 + 1/2 = 1, matching 1/((0+1)(0+1)) = 1 — the decomposition checks out."), difficulty: 5 },
  ],
};
