...
const partialFractionsQuadraticLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...calcChoice("y12e2-calc-pfq-pool-1", "In the partial-fraction decomposition of 1/((x+1)(x²+4)), the term over the irreducible quadratic x²+4 has a numerator of what form?", "B", ["A constant B.", "A linear expression Bx + C.", "A quadratic Bx² + C.", "No numerator is needed."], "An irreducible quadratic factor requires a linear numerator Bx + C, because the numerator over a quadratic can have degree up to one."), difficulty: 2 },
    { ...calcTyped("y12e2-calc-pfq-pool-2", "Decompose 1/((x+1)(x²+1)) = A/(x+1) + (Bx + C)/(x²+1). Find A using the cover-up at x = −1.", "\\frac{1}{(x+1)(x^2+1)} = \\frac{A}{x+1} + \\frac{Bx+C}{x^2+1}", "1/2", ["0.5"], "Cover-up at x = −1: A = 1/((−1)² + 1) = 1/2."), difficulty: 2 },
    { ...calcTyped("y12e2-calc-pfq-pool-3", "For 1/((x+1)(x²+1)) = (1/2)/(x+1) + (Bx + C)/(x²+1), comparing the x² coefficients (0 = A + B) gives B. Find B.", "0 = A + B,\\ A = \\tfrac12", "-1/2", ["−1/2", "-0.5"], "The x² coefficient on the right is A + B and on the left is 0, so B = −A = −1/2."), difficulty: 3 },
    { ...calcTyped("y12e2-calc-pfq-pool-4", "Decompose x/((x+1)(x²+1)) = A/(x+1) + (Bx + C)/(x²+1). Find A using the cover-up at x = −1.", "\\frac{x}{(x+1)(x^2+1)}", "-1/2", ["−1/2", "-0.5"], "Cover-up at x = −1: A = (−1)/((−1)² + 1) = −1/2."), difficulty: 3 },
    { ...calcChoice("y12e2-calc-pfq-pool-5", "A student decomposes 1/((x+1)(x²+1)) as A/(x+1) + B/(x²+1). What is wrong?", "C", ["A should be a linear expression.", "(x²+1) factorises.", "An irreducible quadratic factor needs a LINEAR numerator Bx + C, not a constant B.", "Nothing is wrong."], "Using a constant over x²+1 loses a degree of freedom; the correct numerator is Bx + C so coefficients can be matched."), difficulty: 3 },
    { ...calcTyped("y12e2-calc-pfq-pool-6", "Continuing 1/((x+1)(x²+1)) = (1/2)/(x+1) + (Bx + C)/(x²+1): comparing the constant terms (1 = A + C) gives C. Find C.", "1 = A + C,\\ A = \\tfrac12", "1/2", ["0.5"], "Constant terms: A + C = 1, and A = 1/2, so C = 1/2."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-pfq-pool-7", "The piece (Bx + C)/(x²+1) integrates to (B/2)·ln(x²+1) + C·arctan x. With B = −1/2 and C = 1/2, state the coefficient of arctan x.", "\\frac{Bx+C}{x^2+1} \\to \\tfrac{B}{2}\\ln(x^2+1) + C\\arctan x", "1/2", ["0.5"], "The arctan coefficient is exactly C = 1/2 (the constant part of the numerator)."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-pfq-pool-8", "For 1/((x+1)(x²+1)) the numerator over the quadratic is Bx + C with B = −1/2, C = 1/2. Evaluate Bx + C at x = 1.", "Bx + C,\\ B=-\\tfrac12,\\ C=\\tfrac12", "0", [], "At x = 1: (−1/2)(1) + 1/2 = 0."), difficulty: 4 },
    { ...calcChoice("y12e2-calc-pfq-pool-9", "A student uses a numerator Bx + C over the linear factor (x − 2). What is wrong?", "C", ["Linear factors need a quadratic numerator.", "(x − 2) is irreducible.", "Linear factors get a CONSTANT numerator; only irreducible quadratics get Bx + C.", "Nothing is wrong."], "Each linear factor (x − a) contributes a single constant A/(x − a). Bx + C numerators belong only over irreducible quadratics."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-pfq-pool-10", "Decompose 5/((x−1)(x²+4)) = A/(x−1) + (Bx + C)/(x²+4). Find A using the cover-up at x = 1.", "\\frac{5}{(x-1)(x^2+4)}", "1", [], "Cover-up at x = 1: A = 5/((1)² + 4) = 5/5 = 1."), difficulty: 5 },
    { ...calcTyped("y12e2-calc-pfq-pool-11", "For x²/((x²+1)(x²+4)), substitute u = x² to view it as u/((u+1)(u+4)). Find the coefficient A in A/(u+1) by the cover-up at u = −1.", "\\frac{u}{(u+1)(u+4)},\\ u = x^2", "-1/3", ["−1/3"], "Cover-up at u = −1: A = (−1)/((−1) + 4) = −1/3 (the substitution u = x² turns it into an ordinary cover-up)."), difficulty: 5 },
    { ...calcTyped("y12e2-calc-pfq-pool-12", "After decomposing 1/((x+1)(x²+1)), the antiderivative contains (1/2)ln(x+1) − (1/4)ln(x²+1). Although each log diverges as x → ∞, together they stay finite because they combine as (1/4)ln((x+1)²/(x²+1)). Find the limit of the inside ratio (x+1)²/(x²+1) as x → ∞.", "\\lim_{x\\to\\infty}\\frac{(x+1)^2}{x^2+1}", "1", [], "(x+1)²/(x²+1) = (x²+2x+1)/(x²+1) → 1 as x → ∞ (equal leading degrees), so the combined log tends to (1/4)ln 1 = 0 — the two divergent logs cancel because their arguments grow at the same rate."), difficulty: 5 },
  ],
};
...
const volumesOfRevolutionLesson: Partial<ExplicitLesson> = {
  ...
  masteryQuizPool: [
    { ...calcChoice("y12e2-calc-vol-pool-1", "Rotating the curve y = f(x) about the x-axis, the volume of revolution is V = π × the integral of which expression?", "B", ["y", "y²", "2y", "√y"], "Each cross-section is a disc of radius y and area πy², so V = π × the integral of y² (the disc method)."), difficulty: 2 },
    { ...calcTyped("y12e2-calc-vol-pool-2", "y = √x is rotated about the x-axis from x = 0 to x = 4. Since y² = x, V = π × (the integral of x from 0 to 4). Find V/π.", "V = \\pi\\int_0^4 x\\,dx", "8", [], "∫ x dx from 0 to 4 = [x²/2] = 8, so V/π = 8."), difficulty: 2 },
    { ...calcTyped("y12e2-calc-vol-pool-3", "y = x is rotated about the x-axis from x = 0 to x = 3. V = π × (the integral of x² from 0 to 3). Find V/π.", "V = \\pi\\int_0^3 x^2\\,dx", "9", [], "∫ x² dx from 0 to 3 = [x³/3] = 9, so V/π = 9."), difficulty: 3 },
    { ...calcTyped("y12e2-calc-vol-pool-4", "y = x² is rotated about the x-axis from x = 0 to x = 1. V = π × (the integral of x⁴ from 0 to 1). Find V/π.", "V = \\pi\\int_0^1 x^4\\,dx", "1/5", ["0.2"], "∫ x⁴ dx from 0 to 1 = [x⁵/5] = 1/5, so V/π = 1/5."), difficulty: 3 },
    { ...calcChoice("y12e2-calc-vol-pool-5", "A student finds the volume of y = √x rotated about the x-axis as π × (the integral of √x). What is wrong?", "C", ["The limits are wrong.", "It should be 2π not π.", "The disc radius is y, so the integrand is y² = x, not √x.", "Nothing is wrong."], "The disc area is πy². With y = √x, y² = x, so the integrand is x — the student forgot to square the radius."), difficulty: 3 },
    { ...calcTyped("y12e2-calc-vol-pool-6", "The region between y = x (above) and y = x² (below) on [0, 1] is rotated about the x-axis. The washer volume is V = π × (the integral of (x² − x⁴) from 0 to 1). Find V/π.", "V = \\pi\\int_0^1 (x^2 - x^4)\\,dx", "2/15", ["0.133"], "∫ (x² − x⁴) dx from 0 to 1 = [x³/3 − x⁵/5] = 1/3 − 1/5 = 2/15, so V/π = 2/15."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-vol-pool-7", "Using cylindrical shells, the region under y = x² from x = 0 to x = 2 rotated about the y-axis has V = 2π × (the integral of x·x² from 0 to 2). Find V/(2π).", "V = 2\\pi\\int_0^2 x\\cdot x^2\\,dx", "4", [], "∫ x³ dx from 0 to 2 = [x⁴/4] = 4, so V/(2π) = 4."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-vol-pool-8", "y = eˣ is rotated about the x-axis from x = 0 to x = 1. The disc method gives V = π × (the integral of (eˣ)² from 0 to 1) = (π/2)(e² − c). Find c.", "V = \\pi\\int_0^1 e^{2x}\\,dx = \\tfrac{\\pi}{2}(e^2 - c)", "1", [], "∫ (eˣ)² dx = ∫ e^{2x} dx = [e^{2x}/2] from 0 to 1 = (e² − 1)/2, so c = 1."), difficulty: 4 },
    { ...calcChoice("y12e2-calc-vol-pool-9", "A student rotates the region under y = f(x) about the Y-axis but uses V = π × (the integral of y² dx). Why is that wrong?", "C", ["y² should be y.", "The limits must be negative.", "That formula is for rotation about the x-axis; about the y-axis use shells, V = 2π × (the integral of x·f(x) dx).", "Nothing is wrong."], "π∫y² dx sweeps discs perpendicular to the x-axis. For a y-axis rotation you need shells (2π∫x f(x) dx) or discs written in terms of y."), difficulty: 4 },
    { ...calcTyped("y12e2-calc-vol-pool-10", "y = sin x is rotated about the x-axis from x = 0 to x = π. Using (the integral of sin²x from 0 to π) = π/2, the volume is V = π × (π/2) = a·π². State the coefficient a.", "V = \\pi\\cdot\\tfrac{\\pi}{2}", "1/2", ["0.5"], "V = π × (π/2) = π²/2, so the coefficient of π² is 1/2."), difficulty: 5 },
    { ...calcTyped("y12e2-calc-vol-pool-11", "The region between y = 2 (outer) and y = x (inner) on [0, 2] is rotated about the x-axis. The washer volume is V = π × (the integral of (4 − x²) from 0 to 2). Find V/π.", "V = \\pi\\int_0^2 (4 - x^2)\\,dx", "16/3", ["5.333"], "∫ (4 − x²) dx from 0 to 2 = [4x − x³/3] = 8 − 8/3 = 16/3, so V/π = 16/3."), difficulty: 5 },
    { ...calcTyped("y12e2-calc-vol-pool-12", "y = √(9 − x²) (a semicircle of radius 3) is rotated about the x-axis from x = −3 to x = 3. Recognise that the solid is a SPHERE of radius 3, with known volume (4/3)πr³. State V/π.", "V = \\tfrac{4}{3}\\pi r^3,\\ r = 3", "36", [], "The semicircle generates a sphere of radius 3: V = (4/3)π(3)³ = 36π, so V/π = 36 — recognising the sphere avoids the integral."), difficulty: 5 },
  ],
};
