...
const arithmeticGeometricSeriesLesson = ssLesson(
  ...
  {
    paragraphs: [
      "A series is the sum of terms from a sequence, not just one term, so before any formula try 1+2+3+ up to 10 by hand. Pair the ends: 1 with 10, 2 with 9, 3 with 8, and each pair makes 11, giving five pairs of 11, which is 55. Many HSC errors come from reaching for a term formula when the question actually asks for this kind of running total.",
      "That pairing idea is the whole arithmetic sum formula. Write the sum forwards, then write it again backwards underneath, and add the two lines column by column. Every column now totals the first term plus the last term, and there are n columns, so twice the sum is n lots of (a+l). Halving gives S_n = (n/2)(a+l), which is exactly the average of the first and last terms multiplied by the number of terms.",
      "A geometric series multiplies instead of adds: each term is the running balance times the ratio r, the way a savings balance is scaled each period. To total it, use a subtraction trick. Write the sum S, then write rS, which is the same sum shifted up one power of r. Subtracting S from rS makes almost every term cancel, leaving only ar^n - a, so S(r-1) = a(r^n-1) and S_n = a(r^n-1)/(r-1). This needs r not equal to 1, because a constant series is just na.",
      "Those two derivations give one usable form each, but the geometric form has a matching twin: a(1-r^n)/(1-r) and a(r^n-1)/(r-1) are the same expression with top and bottom multiplied by -1. Use the 1-r version when r<1 so both parts stay positive, and the r-1 version when r>1 so you never divide by a negative. Choose one deliberately for the ratio in front of you rather than switching mid-question.",
      "Sigma notation is a compact instruction to add: the lower number says where to start substituting the index and the upper number says where to stop. In HSC financial questions this same total is disguised as stacking seat rows or accumulating deposits, so decide first whether you need a single term (T_n) or the running total (S_n) before you pick a formula.",
    ],
    latexBlocks: [
      "2S_n=(a+l)+(a+l)+\\cdots+(a+l)=n(a+l)\\ \\Rightarrow\\ S_n=\\frac n2(a+l)=\\frac n2\\left(2a+(n-1)d\\right)",
      "rS_n-S_n=ar^n-a\\ \\Rightarrow\\ S_n=\\frac{a(r^n-1)}{r-1}=\\frac{a(1-r^n)}{1-r}\\quad (r\\ne1)",
  ...
  [
    {
      ...
      steps: [
        { explanation: "The terms add a constant, so this is arithmetic. The pairing form needs the last term, which is not given, so find it first from a and d.", latex: "l=12+19(2)=50" },
        { explanation: "With the first and last terms known, the first-plus-last form is the most direct: average the two ends and multiply by how many terms there are.", latex: "S_{20}=\\frac{20}{2}(12+50)" },
        { explanation: "Evaluate: twenty terms that average 31 each.", latex: "S_{20}=10\\times62=620" },
      ],
      finalAnswerLatex: "S_{20}=620\\ \\text{(the sum of all twenty terms)}",
    },
    {
      title: "Geometric series total",
      questionLatex: "\\text{Find }S_6\\text{ for }a=5\\text{ and }r=2.",
      steps: [
        { explanation: "Each term multiplies by r, so this is geometric, and a total of a fixed number of terms needs the finite sum formula.", latex: "S_n=\\frac{a(1-r^n)}{1-r}" },
        { explanation: "Here r>1, so multiply top and bottom by -1 to use the equivalent form that keeps both parts positive.", latex: "S_n=\\frac{a(r^n-1)}{r-1}" },
        { explanation: "Substitute a=5, r=2 and n=6.", latex: "S_6=\\frac{5(2^6-1)}{2-1}" },
        { explanation: "Evaluate: this is the running total 5+10+20+40+80+160.", latex: "S_6=5(63)=315" },
      ],
      finalAnswerLatex: "S_6=315",
    },
    {
      title: "Expand and evaluate sigma notation",
      questionLatex: "\\sum_{k=1}^{5}(3k+1)",
      steps: [
        { explanation: "The lower index 1 and upper index 5 mean substitute k=1,2,3,4,5 into 3k+1.", latex: "(3+1)+(6+1)+(9+1)+(12+1)+(15+1)" },
        { explanation: "This lists five terms of an arithmetic series with first term 4 and common difference 3.", latex: "4+7+10+13+16" },
        { explanation: "Add by pairing the ends, just like the derivation: 4+16 and 7+13 each make 20, leaving 10 in the middle.", latex: "20+20+10=50" },
      ],
      finalAnswerLatex: "50",
    },
  ],
  ...
  [
    ...
    { mistake: "Dividing by a negative when r>1 in the geometric sum.", fix: "For r>1 use the equivalent form a(r^n-1)/(r-1); it is the same formula with top and bottom multiplied by -1, so both parts stay positive." },
  ],
  [
