...
const networkFundamentals: LessonContent = {
  ...
  masteryQuizPool: [
    ...
    answer("y8-net-fun-p19", "In the network where all 4 vertices A, B, C, D are joined to each other, find the degree of vertex A.", "", "3", "A joins to B, C and D, so $\\deg(A) = 3$.", "Count how many other vertices A connects to.", [], 4),
    answer("y8-net-fun-p20", "A network has 5 vertices, every one joined to every other. Find the degree of any vertex.", "", "4", "Each vertex joins the other 4, so each has degree 4.", "Each vertex connects to all the others.", [], 4),
    answer("y8-net-fun-p21", "A network has 5 vertices, every one joined to every other. How many edges in total?", "", "10", "Each of 5 vertices has degree 4, sum $= 20$, edges $= 20 \\div 2 = 10$.", "Find the degree of each, add, then halve.", [], 4),
    answer("y8-net-fun-p22", "A network has 7 vertices each of degree 2. How many edges does it have?", "", "7", "Sum $= 7 \\times 2 = 14$, edges $= 14 \\div 2 = 7$.", "Multiply, then halve.", [], 4),
    answer("y8-net-fun-p23", "A network has 4 vertices with degrees 3, 3, 3 and an unknown degree d. If it has 6 edges, find d.", "", "3", "Degree sum $= 2 \\times 6 = 12$. So $3+3+3+d = 12$, giving $d = 3$.", "The degree sum is twice the number of edges.", [], 4),
    answer("y8-net-fun-p24", "A network has 6 edges. What is the sum of all its vertex degrees?", "", "12", "Degree sum $= 2 \\times \\text{edges} = 2 \\times 6 = 12$.", "Degree sum is twice the edge count.", [], 4),
    answer("y8-net-fun-p25", "Towns A, B, C, D form a square with roads AB = 5, BC = 5, CD = 5, DA = 5 and a diagonal AC = 7. Find the shorter distance from A to C.", "", "7", "Direct AC $= 7$; via B is $5 + 5 = 10$. The shorter is the diagonal, 7 km.", "Compare the direct edge with the two-edge route.", ["7 km"], 5),
    ...
    answer("y8-net-fun-p27", "A network has 5 vertices with degrees 4, 3, 3, 2 and an unknown d, and 7 edges. Find d.", "", "2", "Degree sum $= 2 \\times 7 = 14$. So $4+3+3+2+d = 14$, giving $d = 2$.", "Use degree sum $= 2 \\times$ edges.", [], 5),
    answer("y8-net-fun-p28", "A 'star' network has 1 centre joined to 6 outer vertices and no other edges. How many edges in total?", "", "6", "The centre connects to 6 outer vertices: 6 edges. (Degree sum $= 6 + 6 \\times 1 = 12$, edges $= 6$.)", "Count the spokes from the centre.", [], 5),
    answer("y8-net-fun-p29", "A network has 8 vertices each of degree 3. How many edges does it have?", "", "12", "Sum $= 8 \\times 3 = 24$, edges $= 24 \\div 2 = 12$.", "Multiply, then halve.", [], 5),
    answer("y8-net-fun-p30", "A network has degree sum 22 and you know one vertex has degree 5. What is the number of edges?", "", "11", "Edges $= 22 \\div 2 = 11$ (the individual degree of 5 does not change the total).", "Halve the degree sum.", [], 5),
  ],
  multiPartPractice: [
    {
      id: "y8-net-fun-mp1",
...
const pathsAndCircuits: LessonContent = {
  ...
  masteryQuizPool: [
    ...
    answer("y8-net-pc-p19", "A network has 5 vertices A–E in a line: AB, BC, CD, DE. How many paths from A to E?", "", "1", "Only $A \\to B \\to C \\to D \\to E$, so 1 path.", "There is just one way along the line.", [], 4),
    answer("y8-net-pc-p20", "Edges AB, AC, BC, CD, BD. How many paths from A to D?", "", "4", "$A \\to B \\to D$, $A \\to C \\to D$, $A \\to B \\to C \\to D$, $A \\to C \\to B \\to D$.", "Direct via each middle vertex, then the two longer routes.", [], 4),
    answer("y8-net-pc-p21", "A network where all of A, B, C, D are joined to each other. How many paths of exactly 2 edges go from A to D?", "", "2", "$A \\to B \\to D$ and $A \\to C \\to D$ are the only 2-edge paths.", "The middle vertex is B or C.", [], 4),
    ...
    answer("y8-net-pc-p23", "A 'fan': vertex A joined to B, C, D, E, and also edges BC, CD, DE. How many paths of exactly 2 edges from A to D? (A–C–D and A–E–D)", "", "2", "Two-edge paths to D: $A \\to C \\to D$ and $A \\to E \\to D$.", "Which neighbours of D are also joined to A?", [], 4),
    ...
    answer("y8-net-pc-p25", "Complete network on 4 vertices A, B, C, D (all pairs joined). How many paths from A to D in total (any length)?", "", "5", "$A\\to D$; $A\\to B\\to D$, $A\\to C\\to D$; $A\\to B\\to C\\to D$, $A\\to C\\to B\\to D$. Total 5.", "Count length 1, then 2, then 3.", [], 5),
    answer("y8-net-pc-p26", "A network has 5 vertices A–E, complete (every pair joined). How many paths of exactly 2 edges from A to E?", "", "3", "Middle vertex is B, C or D: $A\\to B\\to E$, $A\\to C\\to E$, $A\\to D\\to E$ — 3 paths.", "Any of the 3 other vertices can be the middle.", [], 5),
    ...
    answer("y8-net-pc-p28", "Square ABCD with both diagonals (edges AB, BC, CD, DA, AC, BD). How many paths of exactly 2 edges from A to C?", "", "2", "$A \\to B \\to C$ and $A \\to D \\to C$ are the 2-edge paths (the direct AC is length 1).", "Use the two corner routes.", [], 5),
    answer("y8-net-pc-p29", "A line A–B–C–D–E (edges AB, BC, CD, DE). How many paths from B to E?", "", "1", "Only $B \\to C \\to D \\to E$, so 1 path.", "Just one way along the line.", [], 5),
...
const eulerianTrailsCircuits: LessonContent = {
  ...
  masteryQuizPool: [
    ...
    answer("y8-net-eul-p19", "A network has degrees 6, 5, 4, 4, 3, 2. How many odd-degree vertices?", "", "2", "Odd degrees are 5 and 3, so 2 odd-degree vertices.", "Count the odd numbers in the list.", [], 4),
    answer("y8-net-eul-p20", "A connected network is drawn in one stroke and returns to its start. How many odd-degree vertices must it have?", "", "0", "A one-stroke lop returning to start is an Eulerian circuit, which needs 0 odd vertices.", "A returning one-stroke trail is a circuit.", [], 4),
    answer("y8-net-eul-p21", "A connected network can be drawn in one stroke but does NOT return to its start. How many odd-degree vertices?", "", "2", "A one-stroke trail with different start and finish needs exactly 2 odd vertices.", "Different start and finish means a trail, not a circuit.", [], 4),
    ...
    answer("y8-net-eul-p23", "A network has 5 vertices each of degree 4. How many odd-degree vertices, and does it have an Eulerian circuit? Give the number of odd vertices.", "", "0", "All degrees 4 (even), so 0 odd vertices and an Eulerian circuit exists.", "Are any degrees odd?", [], 4),
    ...
    answer("y8-net-eul-p25", "A network has degrees 7, 6, 5, 4, 3, 2, 1. How many odd-degree vertices?", "", "4", "Odd degrees: 7, 5, 3, 1 — that is 4 odd vertices.", "Count the odd numbers.", [], 5),
    ...
    answer("y8-net-eul-p27", "A connected network has 2 odd-degree vertices and 8 edges. Can it be drawn in one stroke? Answer 1 for yes, 0 for no.", "", "1", "2 odd vertices means an Eulerian trail exists, so yes — answer 1. (The 8 edges are irrelevant.)", "Only the odd-vertex count matters.", [], 5),
    ...
    answer("y8-net-eul-p29", "A connected network has 4 odd-degree vertices. What is the least number of separate one-stroke trails needed to cover every edge?", "", "2", "Each trail uses 2 odd vertices, so 4 odd vertices need 4/2 = 2 trails.", "Pair up the odd vertices.", [], 5),
...
const planarGraphs: LessonContent = {
  ...
  masteryQuizPool: [
    ...
    answer("y8-net-pl-p19", "A connected planar network has E = 20, F = 12. Find V.", "", "10", "$V = 20 - 12 + 2 = 10$.", "Use $V = E - F + 2$.", [], 4),
    answer("y8-net-pl-p20", "A connected planar network has V = 12, E = 20. Find F.", "", "10", "$F = 2 - 12 + 20 = 10$.", "Use $F = 2 - V + E$.", [], 4),
    answer("y8-net-pl-p21", "A cube's network (drawn flat) has V = 8 and E = 12. Find the number of faces F.", "", "6", "$F = 2 - 8 + 12 = 6$ — matching a cube's 6 faces.", "Use Euler's formula.", [], 4),
    ...
    answer("y8-net-pl-p23", "A connected planar network has F = 7 and V = 6. Find E.", "", "11", "$E = 6 + 7 - 2 = 11$.", "Use $E = V + F - 2$.", [], 4),
    answer("y8-net-pl-p24", "A connected planar network has E = 9 edges and 5 faces. Find V, then compute V − E + F to verify (give V).", "", "6", "$V = 9 - 5 + 2 = 6$; check $6 - 9 + 5 = 2$. ✓", "Use $V = E - F + 2$.", [], 4),
    answer("y8-net-pl-p25", "A triangular prism network has V = 6 and E = 9. Find F.", "", "5", "$F = 2 - 6 + 9 = 5$ — a triangular prism has 5 faces.", "Use $F = 2 - V + E$.", [], 5),
    answer("y8-net-pl-p26", "A connected planar network has F = 10 faces and V = 7 vertices. Find E.", "", "15", "$E = 7 + 10 - 2 = 15$.", "Use $E = V + F - 2$.", [], 5),
    ...
    answer("y8-net-pl-p28", "A connected planar network has V = 15, E = 25. Find F.", "", "12", "$F = 2 - 15 + 25 = 12$.", "Use $F = 2 - V + E$.", [], 5),
    answer("y8-net-pl-p29", "An octahedron network has V = 6 vertices and F = 8 faces. Find the number of edges E.", "", "12", "$E = 6 + 8 - 2 = 12$ — an octahedron has 12 edges.", "Use $E = V + F - 2$.", [], 5),
    answer("y8-net-pl-p30", "A connected planar network has E = 30 and V = 20. Find F.", "", "12", "$F = 2 - 20 + 30 = 12$.", "Use $F = 2 - V + E$.", [], 5),
  ],
  multiPartPractice: [
    {
      id: "y8-net-pl-mp1",
...
const networkApplications: LessonContent = {
  ...
  masteryQuizPool: [
    ...
    answer("y8-net-app-p19", "From X to Y: XY = 25, X–A–Y = 12+10, X–B–Y = 9+9, X–A–B–Y = 12+3+9. Shortest?", "", "18", "Totals 25, 22, 18, 24; smallest is 18 (via B).", "Add each route carefully.", [], 4),
    answer("y8-net-app-p20", "Five towns in a chain AB = 4, BC = 3, CD = 5, DE = 2 connect all five. Total cost?", "", "14", "$4+3+5+2 = 14$.", "Add the four cables.", [], 4),
    answer("y8-net-app-p21", "Routes A to E: A–B–E = 6+9, A–C–E = 7+7, A–D–E = 5+8. Shortest?", "", "13", "Smallest total is 13 (via D).", "Add each two-hop route.", [], 4),
    answer("y8-net-app-p22", "Four houses AB = 6, AC = 5, AD = 7, BC = 2, CD = 3. Cheapest connector picks AC, BC, CD. Total?", "", "10", "AC=5, BC=2, CD=3 connect all four: $10$ (cheaper than using AB or AD).", "Choose three cheapest connecting cables.", [], 4),
    answer("y8-net-app-p23", "Direct AF = 30, A–B–C–F = 8+7+9, A–D–F = 11+12. Shortest A to F?", "", "23", "Totals 30, 24, 23; smallest is 23 (via D).", "Add all hops on each route.", [], 4),
    ...
    answer("y8-net-app-p25", "From S to T: S–T = 40, S–A–T = 15+18, S–B–T = 14+14, S–A–B–T = 15+5+14. Shortest?", "", "28", "Totals 40, 33, 28, 34; smallest is 28 (via B).", "Add each route; fewer edges isn't always shorter.", [], 5),
    answer("y8-net-app-p26", "Five towns: cheapest connector picks edges of weight 2, 3, 3, 4 (four edges) to connect all five. Total?", "", "12", "$2+3+3+4 = 12$ (5 towns need 4 connecting edges).", "Add the four cheapest connecting edges.", [], 5),
    answer("y8-net-app-p27", "Routes A to D: A–B–D = 9+11, A–C–D = 8+13, A–B–C–D = 9+4+13, A–C–B–D = 8+4+11. Shortest?", "", "20", "Totals 20, 21, 26, 23; smallest is 20 (via B).", "Add every route and compare.", [], 5),
    answer("y8-net-app-p28", "Six towns connected by a minimum connector use 5 cables of weights 3, 4, 4, 5, 6. Total length?", "", "22", "$3+4+4+5+6 = 22$ (6 towns need 5 connecting cables).", "Add the five cables; n towns need n - 1 edges.", [], 5),
    answer("y8-net-app-p29", "Direct XY = 50; X–A–B–Y = 18+9+15; X–C–Y = 22+20; X–A–C–Y = 18+6+20. Shortest X to Y?", "", "42", "Totals 50, 42, 42, 44; the smallest is 42.", "Add each route's weights carefully.", [], 5),
    answer("y8-net-app-p30", "A delivery van must connect depots A, B, C, D, E at least total cost. The cheapest connecting edges are 5, 6, 7, 8. What is the minimum total cost?", "", "26", "$5+6+7+8 = 26$ (5 depots need 4 connecting edges).", "Add the four cheapest connecting edges.", [], 5),
  ],
  multiPartPractice: [
    {
      id: "y8-net-app-mp1",
