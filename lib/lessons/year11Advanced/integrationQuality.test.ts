...
const completedLessonConfigs = [
  {
    slug: "primitives-and-antidifferentiation",
    masteryIdPattern: /^y11adv-intg-prim-qm\d+$/,
  },
  {
    slug: "standard-antiderivatives",
    masteryIdPattern: /^y11adv-intg-std-qm\d+$/,
  },
  {
    slug: "initial-value-problems",
    masteryIdPattern: /^y11adv-intg-iv-qm\d+$/,
  },
  {
    slug: "definite-integrals",
    masteryIdPattern: /^y11adv-intg-def-qm\d+$/,
  },
  {
    slug: "fundamental-theorem-of-calculus",
    masteryIdPattern: /^y11adv-intg-ftc-qm\d+$/,
  },
  {
    slug: "areas-under-curves",
    masteryIdPattern: /^y11adv-intg-area-qm\d+$/,
  },
  {
    slug: "reverse-chain-rule-integration",
    masteryIdPattern: /^y11adv-intg-rcr-qm\d+$/,
  },
  {
    slug: "trapezoidal-rule",
    masteryIdPattern: /^y11adv-intg-trap-qm\d+$/,
  },
] as const;
...
function completedLessons() {
  ...
  return getNewCourseUnitLessons("year-11-advanced", "integration").filter(
    (lesson) => selected.has(lesson.slug),
  );
}
...
test("completed Integration mastery sets meet the rich-task profile", () => {
...
test("completed Integration authored difficulty survives mapping", () => {
...
test("completed Integration practice is unique and operationally complete", () => {
  const lessons = completedLessons();
...
test("completed Integration multipart tasks have worked feedback and answer variants", () => {
