...
const lessonConfigs = [
  { slug: "compound-interest", pattern: /^y11adv-sf-ci-qm\d+$/ },
  { slug: "recursive-formulas-financial", pattern: /^y11adv-sf-rf-qm\d+$/ },
  { slug: "superannuation-future-value", pattern: /^y11adv-sf-sup-qm\d+$/ },
  { slug: "loan-repayments-present-value", pattern: /^y11adv-sf-ln-qm\d+$/ },
  { slug: "series-finance-exam-practice", pattern: /^y11adv-sf-ex-qm\d+$/ },
] as const;
...
function lessons() {
  return getNewCourseUnitLessons("year-11-advanced", "series-finance");
}
...
test("Series and Finance mastery sets meet the rich-task profile", () => {
...
test("Series and Finance authored difficulty survives question-bank mapping", () => {
...
test("Series and Finance practice is unique and operationally complete", () => {
...
test("Series and Finance visuals and critical mathematical keys do not regress", () => {
  ...
  const newVisuals = allQuestions.filter((question) => /^y11adv-sf-.+-qm\d+$/.test(question.id) && (question.dataTableDiagram || question.cartesianGraph));
  assert.equal(newVisuals.length, 4);
  assert.ok(newVisuals.every((question) => ((question.dataTableDiagram ?? question.cartesianGraph)?.description.length ?? 0) >= 30));
  assert.equal(byId.get("y11adv-sf-ci-g3")?.answer, "$6370.54");
  assert.equal(byId.get("y11adv-sf-sup-i3")?.answer, "$65 717.80");
  assert.equal(byId.get("y11adv-sf-ln-i5")?.answer, "≈$75 762.66");
  assert.match(byId.get("y11adv-sf-ex-g3")?.answer ?? "", /1713\.42/);
  assert.equal(byId.get("y11adv-sf-ci-qm10")?.answer, "P>$4721.08 approximately");
  assert.match(byId.get("y11adv-sf-ex-qm9")?.answer ?? "", /Aₙ=0/);
});
