...
const lessonConfigs = [
  { slug: "continuous-random-variables", pattern: /^y11adv-cp-crv-qm\d+$/ },
  { slug: "normal-distribution", pattern: /^y11adv-cp-nd-qm\d+$/ },
  { slug: "z-scores-standardising", pattern: /^y11adv-cp-zs-qm\d+$/ },
  { slug: "normal-tables-probabilities", pattern: /^y11adv-cp-nt-qm\d+$/ },
  { slug: "continuous-probability-exam-practice", pattern: /^y11adv-cp-ex-qm\d+$/ },
] as const;
...
const lessons = () => getNewCourseUnitLessons("year-11-advanced", "continuous-probability");
const normalise = (value: string) => value.toLowerCase().replace(/\s+/g, " ").replace(/[\u2212\u2013\u2014]/g, "-").trim();

test("Continuous Probability mastery sets meet the rich-task profile", () => {
  ...
  for (const lesson of completed) {
    ...
    for (const question of questions) {
      ...
      if (question.choices) {
        ...
        for (const { label } of question.choices) if (label !== question.answer) assert.ok(question.distractorMisconceptions?.[label as "A" | "B" | "C" | "D"], `${question.id} needs a mapping for ${label}`);
      } else {
...
test("Continuous Probability authored difficulty survives question-bank mapping", () => {
  ...
  for (const config of lessonConfigs) {
    ...
    const counts = rows.reduce<Record<number, number>>((result, row) => { result[row.difficulty] = (result[row.difficulty] ?? 0) + 1; return result; }, {});
    assert.equal(rows.length, 10, config.slug);
    assert.deepEqual(counts, { 3: 4, 4: 3, 5: 3 }, config.slug);
  }
});

test("Continuous Probability practice is unique and operationally complete", () => {
  ...
  for (const lesson of lessons()) {
    ...
    for (const question of questions) {
      ...
      if (!question.choices) assert.ok(question.acceptedAnswers?.some((candidate) => candidate.trim() !== question.answer.trim()), `${question.id} needs a legitimate answer variant`);
...
test("Continuous Probability visuals and critical mathematical keys do not regress", () => {
  ...
  const newVisuals = allQuestions.filter((question) => /^y11adv-cp-.+-qm\d+$/.test(question.id) && (question.dataTableDiagram || question.normalDistributionDiagram));
  assert.equal(newVisuals.length, 6);
  assert.ok(newVisuals.every((question) => ((question.dataTableDiagram ?? question.normalDistributionDiagram)?.description.length ?? 0) >= 45));
  assert.match(byId.get("y11adv-cp-ex-i1")?.answer ?? "", /3\/4.*1\/2/);
  assert.match(byId.get("y11adv-cp-nd-i1")?.explanation ?? "", /two standard deviations/);
  assert.match(byId.get("y11adv-cp-crv-qm10")?.answer ?? "", /a=0.*b=1\/2/);
  assert.match(byId.get("y11adv-cp-nt-qm10")?.answer ?? "", /50.*8/);
  assert.match(byId.get("y11adv-cp-ex-qm9")?.answer ?? "", /50.*10.*69\.6/);
  assert.match(byId.get("y11adv-cp-ex-qm10")?.answer ?? "", /1\/2.*sqrt\(2\).*4\/3/);
});
