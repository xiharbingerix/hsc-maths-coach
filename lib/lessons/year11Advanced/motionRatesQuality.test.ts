...
const lessonConfigs = [
  { slug: "displacement-velocity-acceleration", pattern: /^y11adv-mot-da-qm\d+$/ },
  { slug: "finding-displacement-by-integration", pattern: /^y11adv-mot-fd-qm\d+$/ },
  { slug: "motion-analysis", pattern: /^y11adv-mot-ma-qm\d+$/ },
  { slug: "rates-of-change-applications", pattern: /^y11adv-mot-rc-qm\d+$/ },
  { slug: "motion-rates-exam-practice", pattern: /^y11adv-mot-ex-qm\d+$/ },
] as const;
...
function lessons() {
  return getNewCourseUnitLessons("year-11-advanced", "motion-rates");
}
...
test("Motion and Rates mastery sets meet the rich-task profile", () => {
...
test("Motion and Rates authored difficulty survives question-bank mapping", () => {
...
test("Motion and Rates practice is unique and operationally complete", () => {
...
test("Motion and Rates visuals and critical mathematical keys do not regress", () => {
  ...
  const newVisuals = allQuestions.filter((question) => /^y11adv-mot-.+-qm\d+$/.test(question.id) && question.cartesianGraph);

  assert.equal(newVisuals.length, 5);
  assert.ok(newVisuals.every((question) => (question.cartesianGraph?.description.length ?? 0) >= 30));
  assert.equal(byId.get("y11adv-mot-fd-g2")?.answer, "D");
  assert.equal(byId.get("y11adv-mot-ma-i1")?.answer, "22 m");
  assert.equal(byId.get("y11adv-mot-ma-i4")?.answer, "t = 4");
  assert.equal(byId.get("y11adv-mot-ex-i2")?.answer, "B");
  assert.equal(byId.get("y11adv-mot-da-qm8")?.answer, "x(t)=2t^3-12t^2+18t+2");
  assert.match(byId.get("y11adv-mot-ex-qm10")?.answer ?? "", /49\/6/);
});
